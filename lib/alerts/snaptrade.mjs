...
export class SnapTrade {
    constructor(config) {
        ...
        // ── Short-lived read cache — coalesces REPEATED calls to the same endpoint. ──
        this._cache = new Map();               // key → { at, promise, last }
        this._ttl = parseInt(process.env.SNAPTRADE_CACHE_MS || '20000', 10); // 20s default
        // ── Serial throttle — spaces out calls so a burst of DIFFERENT endpoints (e.g. the
        // dashboard's 5 parallel account fetches) doesn't trip SnapTrade's burst rate limit (429s). ──
        this._q = Promise.resolve();
        this._minGapMs = parseInt(process.env.SNAPTRADE_MIN_GAP_MS || '400', 10);
        this._lastCallAt = 0;
    }

    // Run fn serially with a minimum gap since the previous SnapTrade call.
    async _throttled(fn) {
        const run = async () => {
        ...
        const p = this._q.then(run, run); // keep the chain going even if a prior call rejected
        this._q = p.then(() => {}, () => {});
        return p;
    }

    // Memoize a read for _ttl ms; shares an in-flight promise across concurrent callers; on error
    // serves the last good value (stale-on-error) if we have one, else the fallback. Never caches errors.
    async _cached(key, producer, fallback) {
        const now = Date.now();
        const hit = this._cache.get(key);
        if (hit && (now - hit.at) < this._ttl) return hit.promise;
        const promise = this._throttled(producer); // serialize + space out to avoid 429 bursts
        this._cache.set(key, { at: now, promise, last: hit?.last });
        try {
            const val = await promise;
            this._cache.set(key, { at: Date.now(), promise: Promise.resolve(val), last: val });
            return val;
        } catch (err) {
            this._cache.delete(key);
            console.error(`[SnapTrade] ${key} fetch failed: ${err.message}`);
            if (hit?.last !== undefined) { console.warn(`[SnapTrade] serving cached ${key} (stale-on-error).`); return hit.last; }
            return fallback;
        }
    }

    // Clear the read cache — called after any order so the next read reflects the new state.
    _invalidateCache() { this._cache.clear(); }

    async FetchUserTrades() {
        return this._cached('userTrades', async () => {
    ...
    async getBuyDates() {
        return this._cached('buyDates', async () => {
    ...
    async FetchOpenAccountOrders() {
        return this._cached('openOrders', async () => {
 ...
 */
   async PlaceOrder(data) {
    ...
    try {
        const response = await this._throttled(() => this.snaptrade.trading.placeForceOrder(orderPayload));
        this._invalidateCache(); // balance/portfolio/orders changed — force fresh reads next time
    ...
    async FetchAccountBuyingPower() {
        return this._cached('buyingPower', async () => {
    ...
    async FetchAccountTotalValue() {
        return this._cached('totalValue', async () => {
            console.log("[REDLINE] Getting Account Total Value For", this.userId)
            const response = await this.snaptrade.accountInformation.getUserHoldings({
                accountId: this.accountId,
                userId: this.userId,
                userSecret: this.userSecret
            })
            console.log('[REDLINE] Returned Account Total Value Successfully')
    ...
    async FetchAccountOrders24h(onlyExecuted = true) {
        return this._cached(`orders24h:${onlyExecuted}`, async () => {
    ...
    async FetchOrderCompliance() {
        return this._cached('compliance', async () => {
