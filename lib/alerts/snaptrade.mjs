...
async PlaceOrder(data) {
    ...
    // 1. DATA PREP
    // Catch both 'notional' and 'notional_value' naming variations
    const rawNotional = data.notional_value || data.notional;
    const parsedNotional = rawNotional ? parseFloat(rawNotional) : 0;
    const parsedUnits = data.units ? parseFloat(data.units) : 0;
    ...
    // 3. EXCLUSIVE FIELD LOGIC (The Critical Fix)
    // We do NOT set these to null. We only add the one that has a value.
    if (parsedNotional > 0) {
        orderPayload.notional_value = String(parsedNotional);
        // Ensure units key does not exist in the final JSON string
    } else if (parsedUnits > 0) {
        orderPayload.units = parsedUnits;
        // Ensure notional_value key does not exist in the final JSON string
    }

    if (orderPayload.order_type !== "Market" && parsedPrice > 0) {
        orderPayload.price = parsedPrice;
    }

    // FINAL GUARD: If neither units nor notional made it, we can't trade.
    if (!orderPayload.units && !orderPayload.notional_value) {
        console.error(`[REDLINE] ❌ Aborted: No valid quantity found (Units: ${parsedUnits}, Notional: ${parsedNotional})`);
        return null;
    }
    ...
    try {
        ...
        // SnapTrade often returns detailed validation errors in the body
        console.error(`[REDLINE] ❌ SNAPTRADE REJECTED:`, typeof errData === 'object' ? JSON.stringify(errData) : (errData || error.message));
        return null;
    }
}
