...
function evaluatePosition(decision, livePrice, { hhmm }) {
  ...
  // ── 1. INTRADAY EOD force-close ───────────────────────────────────────────
  if (horizon === 'INTRADAY' && hhmm >= INTRADAY_EOD_TIME) {
    return `INTRADAY_EOD — market close at ${INTRADAY_EOD_TIME} ET (P&L: ${pct(pnlPct)})`;
  }
...
async function executeExit(decision, livePrice, reason, portfolioMap, snapTrade, telegramAlerter) {
  ...
  // Resolve the decision in decisions.json
  try {
    resolveDecision(id, {
...
async function runStopLossCheck(snapTrade, telegramAlerter) {
  ...
  // Fetch live portfolio once (for units)
