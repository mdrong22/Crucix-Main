...
// IDs of INTRADAY positions that have been converted to SWING to hold overnight
// Prevents repeated conversions on subsequent ticks
 ...
 *
 * CORE RULE: Never sell a position while P&L is negative.
 *   - Stop-loss threshold reached but P&L < 0 → hold, do not sell.
 *   - Trailing stop activated but P&L < 0 at trigger time → hold, do not sell.
 *   - Only exit when P&L > 0 and trailing stop retracement fires.
 *   - INTRADAY EOD handling is done separately in the main loop (horizon upgrade).
 */
function evaluatePosition(decision, livePrice, { hhmm }) {
  ...
  // ── CORE RULE: never force a sell while the position is at a loss ─────────
  // Stop-loss thresholds are superseded by the recovery-first policy.
  // The position will be held until P&L turns positive or it hits the trailing
  // stop on the way back down from a profitable high.
  if (pnlPct < 0) {
    if (pnlPct <= -thresholds.stopLoss) {
      console.log(
        `[StopLoss] ⏸ HOLD — ${decision.ticker} (${horizon}) hit stop threshold ` +
        `(${pct(Math.abs(pnlPct))} down) but P&L is negative — holding for recovery.`
      );
    }
    return null; // never sell while in the red
  }

  // ── TRAILING STOP (only runs when P&L ≥ 0) ───────────────────────────────
...
async function runStopLossCheck(snapTrade, telegramAlerter) {
  ...
  for (const decision of open) {
    ...
    // ── INTRADAY EOD handling ──────────────────────────────────────────────
    // If P&L is negative at EOD: convert to SWING — hold overnight for recovery.
    // If P&L is positive at EOD: warn to close and take the gain.
    if (decision.horizon === 'INTRADAY' && time.hhmm >= INTRADAY_EOD_TIME) {
      ...
      if (pnlPct < 0 && !_convertedToSwing.has(decision.id)) {
        // Down at EOD — convert to SWING and hold overnight
        _convertedToSwing.add(decision.id);
        if (!decision._synthetic) {
          updateDecision(decision.id, { horizon: 'SWING' });
        }
        console.log(`[StopLoss] 🔄 INTRADAY → SWING — ${ticker} is down ${pct(Math.abs(pnlPct))} at EOD. Holding overnight for recovery.`);
        try {
          telegramAlerter?.sendMessage?.(
            `🔄 INTRADAY → SWING CONVERSION\n` +
            `${ticker} is down ${pct(Math.abs(pnlPct))} at market close.\n` +
            `Holding overnight — will sell when P&L recovers to positive.\n` +
            `Entry: $${parseFloat(decision.entryPrice).toFixed(2)} | Live: $${livePrice.toFixed(2)}`
          );
        } catch (_) {}
        continue; // skip exit evaluation for this tick

      } else if (pnlPct >= 0 && !_warnedEodIds.has(decision.id)) {
        // Up at EOD — warn to close and take the profit
        _warnedEodIds.add(decision.id);
        console.warn(`[StopLoss] ⏰ INTRADAY EOD WARNING — ${ticker} is up ${pct(pnlPct)} at ${time.hhmm} ET. Consider closing to take profit.`);
        try {
          telegramAlerter?.sendMessage?.(
            `⏰ INTRADAY PROFIT ALERT\n` +
            `${ticker} is up 📈 ${pct(pnlPct)} near market close.\n` +
            `Consider closing to lock in gain.\n` +
            `Entry: $${parseFloat(decision.entryPrice).toFixed(2)} | Live: $${livePrice.toFixed(2)}\n` +
