...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    if (isExitDebate) {
      ...
      // ── R/R hard-block ──────────────────────────────────────────────────────
      // Fires on BOTH REJECT and WAIT — Theta can surface an R/R violation in either verdict.
      ...
      // [^\d]* eats any text/punctuation between the keyword and the number (e.g. "ratio (0.80:1)")
      // Previous greedy optional group broke when number was inside parentheses — caused EIS miss.
      const rrMatch = bearThesis.match(/(?:R\/R|R:R|risk[\s\/\-]*reward)[^\d]*([0-9]+\.?[0-9]*)/i);
      const rr = rrMatch ? parseFloat(rrMatch[1]) : null;
      if (rr !== null && rr < rrThreshold && (thetaVerdict === 'REJECT' || thetaVerdict === 'WAIT')) {
        console.log(`[REDLINE] 🛑 R/R hard-block — ${targetTicker}: Theta cited ${rr.toFixed(2)}:1 | ${tradeHorizon} minimum: ${rrThreshold}:1. No score overrides structural R/R failure. Council stands down.`);
        return [{ action: 'WAIT' }];
      }
      if (rr !== null) {
        console.log(`[DEBATE] 📐 R/R check — ${targetTicker}: ${rr.toFixed(2)}:1 (threshold for ${tradeHorizon}: ${rrThreshold}:1) ✅`);
      }
