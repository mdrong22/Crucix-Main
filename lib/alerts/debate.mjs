...
export class Debate {
  ...
  async beginDebate(briefing) {
    ...
    const [portfolio, accountHoldings, accountBalance, orders24h] = await Promise.all([this.snapTradeInstance.FetchUserTrades(), this.snapTradeInstance.getBuyDates(), this.snapTradeInstance.GetAccountBalance(), this.snapTradeInstance.GetAccountOrders24h()])
   const conversation = [
  { 
    role: "system", 
    content: `Current Market Briefing: ${briefing}\nOur Account: ${portfolio} | Our Holdings: ${accountHoldings} | Our Account Balance: ${accountBalance} | Last Orders (24h): ` 
  }
];
    // --- CYCLE 1: THE TARGET ---
