...
import { logDecisions } from "../llm/council/utils/decisionLogger.mjs";
...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    // 7. LOG DECISIONS — Phase 1 feedback loop foundation
    // Persists every actionable verdict to runs/decisions.json with full signal context.
    // briefing = Scout's output text (contains Horizon, Signal Score, Congressional Signal).
    if (sorted.length > 0) {
      try {
        logDecisions(sorted, briefing, liveVix, remainingTrades);
      } catch (err) {
        // Never let logging crash the trade flow
        console.error('[DecisionLogger] Failed to log decisions:', err.message);
      }
    }
