...
export class Debate {
  ...
  _extractVerdict(cleanText) {
    // 1. Standardize the Verdict prefix
    cleanText = cleanText.replace(/(VERDICT:\s*)+/gi, 'VERDICT: ');

    // 2. Capture the JSON block
    // This regex looks for [ or { and grabs everything until it hits the end or a closing char
    ...
    if (verdictMatch && verdictMatch[1]) {
        ...
        const genericMatch = cleanText.match(/([{\[][\s\S]*)/);
        if (genericMatch) jsonToParse = genericMatch[1].trim();
    }

    if (!jsonToParse) return [{ action: "WAIT" }];

    // 3. EMERGENCY BRACKET REPAIR
    // If Gregor sends [{"action":"WAIT"} (chopped), this adds the missing ]
    if (jsonToParse.startsWith('[') && !jsonToParse.endsWith(']')) {
        // Find where the last } is and close the array there
        const lastCurly = jsonToParse.lastIndexOf('}');
        if (lastCurly !== -1) {
            jsonToParse = jsonToParse.substring(0, lastCurly + 1) + ']';
        }
  ...
  async beginDebate(briefing, context) {
    ...
    // 3. ENRICH WITH LIVE QUOTE
    let liveQuote = await this.getLiveQuote(targetTicker);
    const liveContext = `LIVE_PRICE_${targetTicker}: $${liveQuote.price}. Time: ${estTime} EST.`;

    // 4. GREGOR VERDICT (Now expects a list)
    const verdictInstruction = [
      liveContext,
      `ROTATION_RULE: If rotating from one symbol to another, output a LIST: [{"action":"SELL",...}, {"action":"BUY",...}].`,
      `SELL_RULE: If "order_type" is "Limit" (required for EXTENDED sessions), you CANNOT use "notional_value".`,
    ...
    return finalTrades.sort((a, b) => (a.action === "SELL" ? -1 : 1));
  }
}