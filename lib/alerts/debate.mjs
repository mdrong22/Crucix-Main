...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    for (const v of rawVerdicts) {
      ...
      // Minimum viable order check — catch near-zero buying power before hitting SnapTrade
      if (v.action === "BUY") {
        const notional = v.notional_value ?? (v.units && v.price ? v.units * v.price : 0);
        if (notional < MIN_ORDER_NOTIONAL) {
          console.warn(`[DEBATE] 🛑 BUY blocked — ${v.symbol} computed notional $${notional.toFixed(2)} is below minimum $${MIN_ORDER_NOTIONAL}. Insufficient buying power.`);
          continue;
        }
      }
