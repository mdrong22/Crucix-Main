...
export class Debate {
  ...
  async beginDebate(briefing, context) {
    ...
    const bullThesis = await this.bull.assessInfo(`-> Case for ${targetTicker}. consider our portfolio info`, conversation);
    conversation.push({ role: "assistant", name: "Phi", content: bullThesis });

    const bearThesis = await this.bear.complete(`-> Risk case for ${targetTicker} consider our portfolio info`, conversation, { temp: 0.2 });
