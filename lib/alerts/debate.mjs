...
/**
 * Extracts the minimum Phi content Theta needs for its counter-debate.
 * Theta needs: primary bull argument + price levels + R/R ratio.
 * It does NOT need 4,096 tokens of narrative, sizing tables, or forward range prose.
 * Reduces Theta's input by ~70% without losing the signal it acts on.
 */
function extractPhiCore(thesis) {
    // R/R line — critical; Theta's own calc must match or counter it
    const rrLine   = thesis.match(/R[\s\/]*[Rr]\s*=.{0,140}/i)?.[0]?.trim() || '';
    // Price levels Phi cited
    const s1Match  = thesis.match(/S1\s*[=:@]\s*\$?([\d.]+)/i)?.[0] || '';
    const r1Match  = thesis.match(/R1\s*[=:@]\s*\$?([\d.]+)/i)?.[0] || '';
    const stopLine = thesis.match(/[Ss]top\s*[=:@]\s*\$?([\d.]+).{0,60}/)?.[0]?.trim() || '';
    // First 900 chars = primary thesis opening (catalyst + conviction framing)
    const header   = thesis.slice(0, 900);
    const levels   = [s1Match, r1Match, stopLine].filter(Boolean).join(' | ');
    const extras   = [levels ? `LEVELS: ${levels}` : '', rrLine ? `CALC: ${rrLine}` : ''].filter(Boolean).join('\n');
    const result   = header + (extras ? '\n' + extras : '');
    return result.length > 1400
        ? result.slice(0, 1400) + '\n[...Phi full thesis truncated — key R/R and levels extracted above]'
        : result;
}
...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    // ── SCORE GATE — kills low-conviction entries before any LLM or API work ───
    // Performance data (49 resolved): Score ≥ 7 → 70% WR | Score < 7 → 48.3% WR.
    // Horizon-aware thresholds — LONG has 37.5% WR, needs a harder entry bar:
    //   LONG         → ≥ 8  (weeks-months hold; bad entry is expensive to unwind)
    //   SWING        → ≥ 7  (56.8% WR — current gate is working)
    //   INTRADAY     → ≥ 7  (100% WR on 2 trades — leave unchanged)
    // Exit debates are exempt — they inject Signal Score: 10 and have no market-entry signal.
    if (!isExitDebate) {
      const scoreMatch     = briefing.match(/Signal\s*Score:\s*(\d+)/i);
      const signalScore    = scoreMatch ? parseInt(scoreMatch[1], 10) : 0;
      const horizonForGate = briefing.match(/Horizon:\s*(INTRADAY|SWING|LONG)/i)?.[1]?.toUpperCase() || 'SWING';
      const gateThreshold  = horizonForGate === 'LONG' ? 8 : 7;
      if (signalScore < gateThreshold) {
        console.log(`[DEBATE] 🚫 Score gate — Signal Score ${signalScore}/10 < ${gateThreshold} (${horizonForGate} minimum). Skipping council (saves 3 LLM calls). Standing down.`);
        return [{ action: 'WAIT' }];
      }
      console.log(`[DEBATE] ✅ Score gate passed — Signal Score ${signalScore}/10 ≥ ${gateThreshold} (${horizonForGate}). Starting council.`);
    }
    ...
    const liveVix = (() => {
      // context is the compact string from compactSweepForLLM — extract VIXCLS from it
      if (typeof context === 'string') {
        const m = context.match(/VIXCLS=([\d.]+)/);
        if (m) return m[1];
      }
      // Legacy fallback: context as raw array (old code paths)
      try {
    ...
    // ONLY for entry debates. Exit/TRADE-AROUND briefings contain Scout's full output
    // (the exit block PLUS any ESCALATING block from the same sweep), so a naive first-match
    // "Sector:" regex contaminates the exit ticker with another pick's sector.
    // Observed 2026-06-03: BA (exit) mislabeled "energy" from USO's (escalating) Sector: Energy.
    if (!isExitDebate) {
    ...
    // ── Scout price cross-validation ─────────────────────────────────────────
    // Scout embeds its mechanically-verified price as LIVE_PRICE_BA (or LIVE_PRICE_<TICKER>)
    // in the briefing. The debate's getLiveQuote (SnapTrade) is a separate data source and
    // can return stale or incorrect values for some instruments.
    //
    // If the two sources diverge >5%: Scout's price wins.
    // Scout's price comes from the same pipeline that triggered the escalation — it is the
    // price the human or the briefing system saw when the signal fired. Placing an order
    // anchored to a SnapTrade quote that is 80%+ off that price is the exact failure mode
    // that caused the GLD D-grade (Scout $232.44 → SnapTrade $433.25 → limit order at $424.58).
    const scoutPriceMatch =
      briefing.match(/LIVE_PRICE_(?:BA|[A-Z0-9]{1,5}):\s*\$?([\d.]+)/i) ||
      briefing.match(/(?:^|\n)\s*[-–]\s*Price:\s*\$?([\d.]+)/im);
    const scoutVerifiedPrice = scoutPriceMatch ? parseFloat(scoutPriceMatch[1]) : null;

    if (scoutVerifiedPrice && liveQuote?.price) {
      const priceDrift = Math.abs(scoutVerifiedPrice - liveQuote.price) / scoutVerifiedPrice;
      if (priceDrift > 0.05) {
        console.warn(
          `[DEBATE] ⚠ PRICE SOURCE CONFLICT — ${targetTicker}: ` +
          `Scout verified $${scoutVerifiedPrice} vs SnapTrade $${liveQuote.price} ` +
          `(${(priceDrift * 100).toFixed(1)}% drift). ` +
          `Scout price wins — overriding liveQuote for all downstream calcs.`
        );
        liveQuote = { ...liveQuote, price: scoutVerifiedPrice, _source: 'scout_verified' };
      } else {
        console.log(`[DEBATE] ✅ Price sources aligned — ${targetTicker}: Scout $${scoutVerifiedPrice} / SnapTrade $${liveQuote.price} (${(priceDrift * 100).toFixed(1)}% drift, within 5%).`);
      }
    }
    ...
    if (!candleBlock) {
      ...
      if (isExitDebate) {
        ...
        candleBlock = [
          ...
          `  TAKE-PROFIT: $${(lp * 1.02).toFixed(2)} (+2% proxy — for reference only, NOT a sell order).`,
          `  Stop-Loss → use -5% from entry as default ($${(lp * 0.93).toFixed(2)}) until real levels are confirmed.`,
          `  ⛔ Missing data means MINIMUM BUY SIZE — do NOT sell existing holdings to reduce exposure.`,
          `  If Theta flagged missing S1/S2: BUY ORDER SIZE must be halved (existing holdings are unaffected).`,
        ].join('\n');
      }
      console.warn(`[DEBATE] ⚠ No candle data for ${targetTicker} — injecting no-data fallback block. Gregor must use minimum sizing.`);
    }

    // Build lean anchor conversation — candle data + key params only.
    // The full Scout briefing is passed to Phi directly via its scoutBriefing param.
    // Theta and Gregor never need the raw market sweep — they act on candles + Phi/Theta output.
    // This is the single biggest token saver: removes 5,000-8,000 briefing tokens from Theta's context.
    const _sigScore = briefing.match(/Signal\s*Score:\s*(\d+)/i)?.[1] || '?';
    const conversation = [
      { role: "user", content: `TARGET: ${targetTicker} | HORIZON: ${tradeHorizon} | VIX: ${liveVix} | SIGNAL_SCORE: ${_sigScore}${candleBlock}` },
    ];
    ...
    // Pass only Phi's core (primary arg + levels + R/R) to Theta — not the full narrative.
    // Theta must counter Phi's R/R claim and primary risk; it doesn't need sizing tables or prose.
    const phiForTheta = extractPhiCore(bullThesis);
    console.log(`[DEBATE] 📋 Phi core for Theta: ${phiForTheta.length} chars (was ${bullThesis.length} chars)`);
    conversation.push({ role: "assistant", name: "Phi", content: phiForTheta });
    ...
    if (isExitDebate) {
      ...
      // ── R/R hard-block ──────────────────────────────────────────────────────
      // UNCONDITIONAL — fires regardless of Theta's verdict phrasing.
      // Horizon-aware thresholds (calibrated against 49-decision performance review):
      //   SWING    → ≥ 1.5:1  (56.8% WR — gate working correctly)
      //   INTRADAY → ≥ 1.0:1  (100% WR on tight windows; strict R/R would kill valid scalps)
      //   LONG     → ≥ 2.0:1  (37.5% WR at 1.0:1 was the failure — needs structural room)
      //             At 37.5% WR you need R/R ≥ 1.67:1 to break even; 2.0:1 is the safety margin.
      // No signal score overrides a structural R/R failure.
      const rrThreshold = tradeHorizon === 'SWING' ? 1.5 : tradeHorizon === 'LONG' ? 2.0 : 1.0;
      const RR_REGEX = /(?:R\/R|R:R|risk[\s\/\-]*reward)[^\d]*([0-9]+\.?[0-9]*)/i;
      // Parse R/R from BOTH Phi (bull) and Theta (bear) — use the lower (more conservative) value.
      // Phi admitted 0.80:1 in the GLD post-mortem; Theta's verdict phrasing was non-standard.
      const rrMatchTheta = bearThesis.match(RR_REGEX);
      const rrMatchPhi   = bullThesis.match(RR_REGEX);
      const rrRaw = [rrMatchTheta, rrMatchPhi]
        .filter(Boolean)
        .map(m => parseFloat(m[1]))
        .filter(v => !isNaN(v) && v > 0);
      const rr = rrRaw.length > 0 ? Math.min(...rrRaw) : null;

      // Mechanical R/R from candle data — fallback when LLM text parsing fails.
      // Uses S1 as entry proxy and R1 as target (same anchors Phi/Theta use).
      // Stop = S2 if available, else S1 - ATR.
      let mechanicalRR = null;
      if (technicalCtx.s1 != null && technicalCtx.r1 != null && technicalCtx.price != null) {
        const entry  = technicalCtx.s1;   // proxy BUY limit
        const target = technicalCtx.r1;
        const stop   = technicalCtx.s2 ?? (technicalCtx.s1 - (technicalCtx.atr ?? technicalCtx.s1 * 0.03));
        const reward = target - entry;
        const risk   = entry - stop;
        if (risk > 0 && reward > 0) mechanicalRR = reward / risk;
      }
      const effectiveRR = (rr !== null && mechanicalRR !== null) ? Math.min(rr, mechanicalRR)
                        : (rr !== null ? rr : mechanicalRR);

      if (effectiveRR !== null && effectiveRR < rrThreshold) {
        const src = rr !== null && mechanicalRR !== null ? `LLM ${rr.toFixed(2)} / Mechanical ${mechanicalRR.toFixed(2)}`
                  : rr !== null ? `LLM ${rr.toFixed(2)}` : `Mechanical ${mechanicalRR.toFixed(2)}`;
        console.log(`[REDLINE] 🛑 R/R hard-block — ${targetTicker}: ${src} < ${rrThreshold}:1 (${tradeHorizon} minimum). No score overrides structural R/R failure. Standing down.`);
        return [{ action: 'WAIT' }];
      }
      if (effectiveRR !== null) {
        console.log(`[DEBATE] 📐 R/R OK — ${targetTicker}: ${effectiveRR.toFixed(2)}:1 ≥ ${rrThreshold}:1 (${tradeHorizon}) ✅`);
      }

      // ── No candle data + REJECT = hard block ────────────────────────────────
      // Proxy R/R (+2% gain / -5% stop ≈ 0.4:1) always fails SWING threshold.
      // Don't burn a Gregor call when outcome is structurally predetermined.
      if (!technicalCtx.s1 && thetaVerdict === 'REJECT') {
        console.log(`[REDLINE] 🛑 No candle data + Theta REJECT — proxy R/R is structurally sub-threshold for ${tradeHorizon}. Standing down.`);
        return [{ action: 'WAIT' }];
      }

      // REJECT hard-blocks signals below the high-conviction threshold.
      // Raised from 6 → 8: a REJECT from Theta should require strong conviction to override.
      if (thetaVerdict === 'REJECT' && signalScore < 8) {
        console.log(`[REDLINE] 🛑 Theta VETO — verdict: REJECT, score: ${signalScore}/10 (must be ≥ 8 to escalate over REJECT). Council stands down.`);
        return [{ action: 'WAIT' }];
      }
      if (thetaVerdict === 'REJECT') {
        console.warn(`[REDLINE] ⚠ Theta REJECT but score ${signalScore} ≥ 8 — escalating to Gregor with veto flag.`);
      }
      ...
      // ── Null verdict guard — Theta output was truncated or malformed ──────────
      // thetaVerdict is null when Theta's response didn't contain a THETA VERDICT: line
      // (most common cause: 2048-token truncation cuts off the last line).
      // Without a verdict we can't apply REJECT/WAIT veto logic — fall back on structure.
      if (thetaVerdict === null) {
        console.warn(`[REDLINE] ⚠ ${targetTicker}: Theta verdict unparseable — response may be truncated.`);
        if (!technicalCtx.s1) {
          // No candle data + no verdict = can't verify R/R structurally → hard block
          console.log(`[REDLINE] 🛑 Null verdict + no candle data — cannot verify R/R. Standing down.`);
          return [{ action: 'WAIT' }];
        }
        // Candle data present: mechanicalRR check already ran above. If it passed, continue.
        console.warn(`[REDLINE] ⚠ Null verdict but mechanicalRR check passed — escalating to Gregor without veto flag.`);
      }

      // WAIT hard-blocks: LONG requires score ≥ 9 to override (structural thesis must be ironclad),
      // SWING/INTRADAY require ≥ 7. LONG at 37.5% WR means Theta WAIT on a LONG is almost always right.
      const waitOverrideThreshold = tradeHorizon === 'LONG' ? 9 : 7;
      if (thetaVerdict === 'WAIT' && signalScore < waitOverrideThreshold) {
        console.log(`[REDLINE] 🛑 Theta WAIT + score ${signalScore} < ${waitOverrideThreshold} (${tradeHorizon} override threshold) — insufficient conviction. Council stands down.`);
        return [{ action: 'WAIT' }];
      }
      if (thetaVerdict === 'WAIT') {
        console.warn(`[REDLINE] ⚠ Theta WAIT but score ${signalScore} ≥ ${waitOverrideThreshold} (${tradeHorizon}) — escalating to Gregor with wait flag.`);
      }
    }

    // NOTE: candleBlock is already in conversation[0] — do NOT repeat it here to avoid
    // Gregor seeing duplicate S1/R1/ATR numbers (wastes ~300 tokens and can confuse sizing).
    ...
    const verdictInstruction = [
      liveContext,
      isExitDebate
        ? `EXIT MODE — MANDATORY SYMBOL: ${targetTicker}. This is a SELL order for ${targetTicker} ONLY — the held position being managed. You MUST output "symbol":"${targetTicker}". Do NOT substitute, rotate to, or invent any other ticker. If your reasoning mentions a different symbol, that is an error — the only valid symbol is ${targetTicker}. Win-rate modifiers and signal-score sizing rules DO NOT apply. Execute the sell at the stated price and units. Output WAIT only if Phi or Theta flagged the sell target as genuinely unreachable.`
        : '',
      isExitDebate
        ? `⛔ ROTATION_RULE is DISABLED in EXIT MODE. Output exactly ONE order: a SELL for ${targetTicker}. No BUY leg, no second symbol.`
        : '',
    ...
    const MIN_ORDER_NOTIONAL  = 10.00; // USD — $1.75 "trades" (EIS post-mortem) are modifier math errors, not real entries
    // Rotation = a SELL paired with a BUY in the same verdict batch (swap one position for another).
    // A standalone SELL (no accompanying BUY) is NOT a rotation — it's an exit, and must come
    // from an exit-debate. Incorrectly marking standalone SELLs as rotations bypasses the
    // MIN_ORDER_NOTIONAL check and signals freed capital for a BUY that never materialises.
    const isRotation = rawVerdicts.some(v => v.action === "SELL") && rawVerdicts.some(v => v.action === "BUY");
    const finalTrades = [];
    for (const v of rawVerdicts) {
      if (v.action === "WAIT") continue;

      // Block unsolicited SELL verdicts on entry debates.
      // Entry debates are triggered by BUY signals — Gregor must WAIT if uncertain, not sell
      // existing holdings. Legitimate exits must go through exit-debate mode (isExitDebate=true).
      // Rotation SELLs (paired with a BUY) are still allowed.
      if (v.action === "SELL" && !isExitDebate && !isRotation) {
        console.warn(`[DEBATE] ⛔ Blocked unsolicited SELL on entry debate for ${v.symbol || targetTicker} — use exit-debate mode to close positions. Gregor should WAIT, not sell existing holdings.`);
        continue;
      }
