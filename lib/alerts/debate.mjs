...
export class Debate {
  ...
  _extractVerdict(text) {
    ...
    try {
      // Clean up common LLM JSON errors like trailing commas
  ...
  `CRITICAL RULES:`,
  `1. If trading during EXTENDED hours (4:00 PM - 8:00 PM ET), you MUST use order_type: "Limit" and provide a price. Current Time ${new Date().toISOString()} to EST`,
  `2. For fractional trades, prefer "notational_value" (dollar amount) over "units" (share count) for Market orders.`,
  `3. You must provide EITHER "units" OR "notational_value", the other must be null.`,
  `Required fields:`,
  ...
  `  - notational_value:   dollar amount (positive number or null)`,
  `  - trading_session:  "REGULAR" | "EXTENDED"`,
  `Output your reasoning in 1-2 sentences, then end with:`,
  `VERDICT: {"action":"...","order_type":"...","time_in_force":"...","price":...,"units":...,"notational_value":...,"trading_session":"..."}`,
