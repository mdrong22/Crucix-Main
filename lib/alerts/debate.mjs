...
export class Debate {
  ...
  async beginDebate(briefing, portfolio, holdings) {
    ...
    const conversation = [
      { role: "system", content: "Current Market Briefing: " + briefing },
      { role: "system", content: `Account: ${portfolio} | Holdings: ${holdings}` }
    ];

    // --- CYCLE 1: THE TARGET ---
    const selectionMsg = "Identify ONE priority ticker. Format: TARGET: [TICKER]";
    const selection = await this.bull.complete(selectionMsg, briefing); 
    ...
    const gregorResponse = await this.gregor.complete(
      `${GregorSysPrompt} -> Based on the debate, output the final trade parameters. 
