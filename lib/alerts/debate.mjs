...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    // WAIT is softer than REJECT but must still be explicitly addressed by Gregor.
    // Score < 8 on a WAIT = hard block (not enough conviction to override caution).
    if (thetaVerdict === 'WAIT' && signalScore < 8) {
      console.log(`[REDLINE] 🛑 Theta WAIT + score ${signalScore} < 8 — insufficient conviction to override. Council stands down.`);
      return [{ action: 'WAIT' }];
    }
    if (thetaVerdict === 'WAIT') {
      console.warn(`[REDLINE] ⚠ Theta WAIT but score ${signalScore} ≥ 8 — escalating to Gregor with wait flag.`);
    }
    ...
    const verdictInstruction = [
      liveContext,
      thetaVerdict === 'REJECT' ? `⚠ THETA VETO FLAG: Theta issued REJECT but signal score (${signalScore}) overrides the veto threshold. Address Theta's concern explicitly in Logic — name it, counter it, accept the risk in writing.` : '',
      thetaVerdict === 'WAIT'   ? `⚠ THETA WAIT FLAG: Theta issued WAIT (score ${signalScore} clears the override threshold). You MAY proceed, but you MUST name Theta's specific primary risk in Logic under "Bear Response" and state why you are overriding it. If you cannot provide a concrete counter, output WAIT.` : '',
