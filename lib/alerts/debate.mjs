...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    // Entering a trade where risk > reward is never justified regardless of signal strength.
    // Theta's verdict rules: "REJECT only when R/R < 1:1" — enforce that here in code.
    ...
    // A score of 6+ on a REJECT still goes to Gregor — he sees the veto flag and must address it.
    // Rationale: Scout calibrated to 6+ for INTRADAY/catalyst setups; Theta shouldn't silently kill these.
    ...
    // ── Undefined stop-loss hard-block (WAIT or REJECT) ─────────────────────
    // If Theta explicitly states an undefined stop — no S1/S2/ATR data — entering the
    // trade is entering with unknown downside. No signal score or bull thesis resolves that.
    // This fires before the score threshold checks so score cannot override it.
    ...
    // WAIT hard-blocks at score < 7 (lowered from 8).
    // A score of 7+ with Theta WAIT still goes to Gregor with the wait flag.
    ...
    const verdictInstruction = [
      liveContext,
      thetaVerdict === 'REJECT' ? `⚠ THETA VETO FLAG: Theta issued REJECT. Signal score (${signalScore}) clears the hard-block threshold, so you may evaluate — but you are NOT cleared to proceed automatically. You MUST identify Theta's PRIMARY RISK (the specific named risk, not a generic restatement), provide a concrete data-point counter to it, and confirm R/R ≥ 1:1 before proceeding. "Signal score overrides the threshold" is NOT a valid justification — it is circular. If you cannot name a specific counter to Theta's primary risk, or if R/R < 1:1, output WAIT.` : '',
      thetaVerdict === 'WAIT'   ? `⚠ THETA WAIT FLAG: Theta issued WAIT (score ${signalScore} clears the override threshold). You MAY proceed, but you MUST name Theta's specific primary risk in Logic under "Bear Response" and state why you are overriding it with a concrete counter-argument. If you cannot provide a concrete counter, output WAIT.` : '',
