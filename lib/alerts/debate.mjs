...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    // 4. ENRICH WITH LIVE QUOTE + CANDLE DATA
    let liveQuote;
    try {
    ...
    // Fetch 5-min candles for realistic limit price anchoring
    let candleBlock = '';
    try {
      const tech = await getHistoricalTechnicals(targetTicker, '5Min', 20);
      if (tech?.bars?.length) {
        const highs  = tech.bars.map(b => b.h);
        const lows   = tech.bars.map(b => b.l);
        const recentHigh = Math.max(...highs).toFixed(2);
        const recentLow  = Math.min(...lows).toFixed(2);
        const rsi    = tech.rsi != null ? tech.rsi.toFixed(1) : 'N/A';
        const mom    = tech.momentum ?? 'N/A';
        candleBlock = [
          ``,
          `CANDLE DATA — ${targetTicker} (5-min bars, last ${tech.bars.length} bars):`,
          `  Current price : $${liveQuote.price}`,
          `  RSI(14)       : ${rsi}`,
          `  Momentum      : ${mom}%`,
          `  Recent high   : $${recentHigh}  ← BUY limit ceiling / SELL limit target`,
          `  Recent low    : $${recentLow}  ← BUY limit target / SELL limit floor`,
          ``,
          `PRICING RULE (MANDATORY):`,
          `  BUY  limit → set between recentLow ($${recentLow}) and current price ($${liveQuote.price}). Aim for support, not current ask.`,
          `  SELL limit → set between current price ($${liveQuote.price}) and recentHigh ($${recentHigh}). Aim for resistance.`,
          `  Never set a BUY limit above current price. Never set a SELL limit below current price.`,
        ].join('\n');
        console.log(`[DEBATE] 📊 Candle data for ${targetTicker}: high=$${recentHigh} low=$${recentLow} RSI=${rsi}`);
      }
    } catch (err) {
    ...
    const ROTATION_FLOOR_PCT = -3.0; // position must be down ≥3% to qualify as rotation sell
    const finalTrades = [];
    for (const v of rawVerdicts) {
      if (v.action === "WAIT") continue;

      // Rotation guard — mechanical, not prompt-based.
      // Block any SELL on a held position unless it's actually down ≥3%.
      // Prevents LLM from rotating out of positions on noise (bad P&L data, rounding, etc.)
      if (v.action === "SELL") {
        const held = cleanedPortfolio.find(p => p.symbol === v.symbol);
        if (held) {
          const pnlPct = parseFloat(held.pnl_pct); // e.g. -0.36%
          if (!isNaN(pnlPct) && pnlPct > ROTATION_FLOOR_PCT) {
            console.warn(`[DEBATE] 🛑 Rotation SELL blocked — ${v.symbol} P&L: ${pnlPct.toFixed(2)}% (floor: ${ROTATION_FLOOR_PCT}%). Dropping trade.`);
            continue;
          }
        }
      }
