...
export class Debate {
  ...
  async beginDebate(briefing, context, remainingTrades) {
    ...
    // Performance review (31 decisions): Score ≥ 7 → 50% WR | Score < 7 → 28.6% WR.
    // A Score < 7 debate burns Phi + Theta + Gregor (3 LLM calls) with negative expected value.
    ...
    if (!isExitDebate) {
      ...
      if (signalScore < 7) {
        console.log(`[DEBATE] 🚫 Score gate — Signal Score ${signalScore}/10 < 7. Skipping council (saves 3 LLM calls). Standing down.`);
        return [{ action: 'WAIT' }];
      }
      console.log(`[DEBATE] ✅ Score gate passed — Signal Score ${signalScore}/10 ≥ 7. Starting council.`);
    }
    ...
    if (isExitDebate) {
      ...
      //   SWING    → requires ≥ 1.5:1  (multi-day holds demand higher reward cushion)
      //   INTRADAY → requires ≥ 1.0:1  (tight windows, quick exits)
      //   LONG     → requires ≥ 1.0:1  (position-level entry, thesis absorbs more draw)
      // No signal score overrides a structural R/R failure.
      const rrThreshold = tradeHorizon === 'SWING' ? 1.5 : 1.0;
