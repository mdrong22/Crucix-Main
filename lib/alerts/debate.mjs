...
export class Debate {
  ...
  async beginDebate(briefing, context) {
    ...
    const [port, buyingPower, totalValue, orders, openAccOrders] =
      ...
      const [portfolio, orders24h, openAccountOrders] = [DataCleaner.stringifyPortfolio(port) || "NONE", DataCleaner.stringifyOrders(orders) || "NONE", DataCleaner.stringifyOpenOrders(openAccOrders) || "NONE"];
    ...
    console.log(`[REDLINE] ${JSON.stringify(verdict)}`)
