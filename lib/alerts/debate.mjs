...
export class Debate {
  ...
  _extractVerdict(text) {
    ...
    const match = stripped.match(/VERDICT:\s*({[\s\S]*?})/i);
    
    if (!match) {
      console.warn("[REDLINE] ⚠️ Gregor produced no VERDICT block. Raw output:", text.slice(0, 100));
      return { action: "WAIT" };
    }
    ...
    try {
      ...
      console.error("[REDLINE] ❌ Gregor JSON parse failed:", match[1]);
      return { action: "WAIT" };
    }
  }
  ...
  _isPriceSafe(gregorPrice, livePrice, maxDriftPct = 0.02) {
    ...
    if (drift > maxDriftPct) {
      console.warn(`[REDLINE] ⚠️ Price drift detected: Gregor=$${gregorPrice} | Live=$${livePrice}`);
      return false;
    }
    return true;
  }
  ...
  async beginDebate(briefing, context) {
    ...
    const historicalData = await getHistoricalTechnicals(targetTicker)
    console.log(`[REDLINE] 🎯 Council target: ${targetTicker}`);
    conversation.push({ role: "assistant", name: "Phi", content: `Target: ${targetTicker} historical data (5min): ${JSON.stringify(historicalData)}` });
    ...
    const verdictInstruction = [
      ...
      `CRITICAL RULES:`,
      `1. If trading during EXTENDED hours (4:00 PM - 8:00 PM EST) (4:00 AM - 9:30 AM EST), you MUST use order_type: "Limit" and provide a price, and time_in_force: "Day". Current Time ${new Date().toISOString()} to EST`,
      ...
      `4. Notional_value can only work with 'Market' for 'order_type' and 'Day' for 'time_in_force'.`,
      `Required fields:`,
      `5. EXTENDED HOURS REQUIRE WHOLE UNITS. You cannot use "notional_value" or fractional decimals (e.g. 0.5) for EXTENDED/LIMIT orders. Calculate units as floor(buying_power / price).`,
    ...
    return {
      ...
      notional_value: verdict.notional_value ?? null,
      transcript: conversation,
    };
  }
}
