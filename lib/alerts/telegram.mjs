...
export class TelegramAlerter {
  constructor({ botToken, chatId, proposalsChatId, snapTradeInstance }) {
    this.botToken = botToken;
    this.chatId = chatId;
    this.proposalsChatId = proposalsChatId || chatId; // dedicated channel for trade proposals
    ...
    this._callbackHandler = null; // Registered inline-button (Accept/Deny) callback
    this._pollingInterval = null;
    this._botUsername = null;
    this.snapTrade = snapTradeInstance;
    this.lastGeopoliticalSummary = null; // Latest LLM geopolitical reason — read by server → Scout
  }
      ...
      for (let i = 0; i < chunks.length; i++) {
        const res = await fetch(`${TELEGRAM_API}/bot${this.botToken}/sendMessage`, {
          ...
          body: JSON.stringify({
            ...
            // Inline keyboard (e.g. Accept/Deny) only on the last chunk so buttons sit under the full text.
  ...
  /** Delete a message (powers "expired proposals disappear"). */
  async deleteMessage(messageId, chatId = this.proposalsChatId) {
    if (!this.isConfigured || !messageId) return false;
    try {
      const res = await fetch(`${TELEGRAM_API}/bot${this.botToken}/deleteMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  /** Edit an existing message's text/buttons (e.g. card → "✅ Executed" / "❌ Denied"). */
  async editMessageText(messageId, text, opts = {}) {
    if (!this.isConfigured || !messageId) return false;
    const chatId = opts.chatId ?? this.proposalsChatId;
    try {
      const res = await fetch(`${TELEGRAM_API}/bot${this.botToken}/editMessageText`, {
        ...
        body: JSON.stringify({
          ...
          reply_markup: opts.replyMarkup ?? { inline_keyboard: [] }, // clears buttons by default
        }),
        signal: AbortSignal.timeout(10000),
      });
      return res.ok;
  ...
  /**
   * Register the inline-button handler. Called with (action, id, ctx) where action is 'accept'|'deny',
   * id is the proposal id, ctx = { messageId, chatId, callbackQueryId }. Should return an optional
   * short toast string.
   */
  onCallback(handler) {
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    if (llmProvider?.isConfigured) {
      try {
        const portfolio = await this.snapTrade.GetCurrentPortfolio()
        const systemPrompt = this._buildEvaluationPrompt(portfolio);
    ...
    // Capture geopolitical summary for Scout context (regardless of alert outcome)
    if (evaluation?.reason) {
      this.lastGeopoliticalSummary = evaluation.reason;
    }
  ...
  async _pollUpdates() {
    try {
      ...
      for (const update of data.result) {
        ...
        // Inline-button taps (Accept/Deny on a proposal card)
        ...
        // Accept commands from either the main chat or the proposals channel.
        if (chatId !== String(this.chatId) && chatId !== String(this.proposalsChatId)) continue;
  ...
  async _handleCallback(cbq) {
    ...
    // Only honour taps from the proposals channel (or main chat).
    if (chatId !== String(this.chatId) && chatId !== String(this.proposalsChatId)) return;
    const data = String(cbq.data || ''); // "accept:<id>" | "deny:<id>"
    const [action, id] = data.split(':');
    if ((action !== 'accept' && action !== 'deny') || !id) { await this._answerCallback(cbq.id); return; }

    if (!this._callbackHandler) { await this._answerCallback(cbq.id, 'Not ready'); return; }
    try {
      const toast = await this._callbackHandler(action, id, {
        messageId: cbq.message?.message_id,
        chatId,
        callbackQueryId: cbq.id,
      });
      await this._answerCallback(cbq.id, toast || (action === 'accept' ? 'Accepted' : 'Denied'));
    } catch (err) {
      console.error('[Telegram] callback handler error:', err.message);
      await this._answerCallback(cbq.id, 'Error');
    }
  }
  ...
  _buildEvaluationPrompt(userTrades) {
...
Respond with ONLY valid JSON:
{
  ...
  "actionable": "Specific action the user could take based on their portfolio or global events (or 'Monitor' if just informational)",
  ...
  _formatTieredAlert(evaluation, delta, tier) {
   ...
   async sendTradeAlert(trade) {
    // 1. Determine the "Amount" string based on what was actually sent
    let amountDisplay = "";
    
    if (trade.notional_value) {
        amountDisplay = `$${trade.notional_value} worth`;
    } else if (trade.units) {
        amountDisplay = `${trade.units} units`;
    } else {
        amountDisplay = "a position";
    }
  
    // 2. Format the action emoji
    const emoji = trade.action === 'BUY' ? '🚀' : '⚖️';
    const actionText = trade.action === 'BUY' ? 'BOUGHT' : 'SOLD';
  
    // 3. Optional: Add a "Reason" or "Story" if your trade object carries it
    const story = trade.story ? `\n\n*The Story:* ${trade.story}` : "";
    ...
    *Type:* ${trade.order_type || 'Market'} (${trade.trading_session || 'REGULAR'})${story}
  
    _Status: Executed via Project RedLine_
    `.trim();
  
    // Call your existing telegram client
...
export function formatToTelegramMarkdown(text) {
  if (!text) return "";

  return text
    // 1. Remove all existing backslashes as requested
    .replace(/\\/g, '') 
    
    // 2. Handle Asterisks:
    // - Ignore '**' (Bold)
    // - Ignore '* ' at the start of a line (Bullet)
    // - Escape all other single asterisks to prevent 400 errors
    .replace(/(?<!^)(?<!\*)\*(?!\*| )/gm, '\\*')

    // 3. Escape other V1 reserved characters that aren't bold/bullets
    // We escape [ and ` (keeping _ for italics if you use them)
    .replace(/[[`]/g, '\\$&'); 
}
