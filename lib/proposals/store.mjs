...
/**
 * Count proposals SURFACED today (ET calendar day), optionally of a given action.
 * Counts every status (a denied/expired proposal still "cost" you a decision), so this is a
 * true ceiling on daily churn. Used to enforce the daily new-buy cap (quality over quantity).
 */
export function countToday(action = null) {
  const today = new Date().toLocaleDateString('en-US', { timeZone: 'America/New_York' });
  return loadProposals().filter(p => {
    if (action && p.action !== action) return false;
    const d = new Date(p.createdAt).toLocaleDateString('en-US', { timeZone: 'America/New_York' });
    return d === today;
  }).length;
}
