...
export class ScoutLLM extends CouncilAgent {
    ...
    async assessInfo(context, currentData, userPort, lastDecision, buyingPower, openAccOrders) {
        ...
        1. PORTFOLIO & ORDER DEFENSE:
           ...
           - CRITICAL: You are FORBIDDEN from recommending a ticker that already has an "ACCEPTED" or "WORKING" order in ${openAccountOrders}. 
           - If an order for a Target is already open, you must pivot to the next strongest lead or set STATUS: QUIET.
        
        2. ACTIVE HUNTING & SELECTION:
        ...
        3. EXECUTION VIABILITY (The "No Money" Protocol):
           - IF REGULAR_MARKET_OPEN: Fractional allowed. If (Buying Power < Price), proceed with Notional entry.
           - IF EXTENDED_HOURS_RESTRICTED: Whole shares ONLY. 
           - CRITICAL: If (Buying Power < Price) during Extended Hours, you are FORBIDDEN from choosing "QUIET" until you scan ${userPortfolio} for a Rotation. 
           - ROTATION RULE: If a new lead is stronger than your "Weakest Link" (lowest 24h momentum or profit target hit), you MUST set STATUS: ESCALATING and define a "Rotation_Target".
           - If there are no rotations to a new stock and not enough buying power for a whole share during EXTENDED HOURS => QUIET PROTOCOL
        
        4. QUIET MODE:
           - Only allowed if NO ideas meet Technical/News triggers OR no viable rotation exists OR all viable targets already have OPEN_ORDERS.
        ...
        - **Rotation_Target:** [Ticker to SELL, else "None"]
        - **Trigger:** [Specific reason: e.g., "RSI Breakout + News" or "Average Down"]
        - **The Data:** [Price, RSI, VIX Level: ${vix}, 5m Momentum, Latest Close]
        - **The Story:** [2-3 sentence macro context]
        - **Scout's Note:** [Why this ticker? Why sell the Rotation_Target now? Cite specific %, bars, and momentum tags provided in live data. Acknowledge openAccountOrders: ${openAccountOrders? openAccountOrders: 'NONE'} if it blocked a previous target.]`;
        try {
