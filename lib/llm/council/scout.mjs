...
export class ScoutLLM extends CouncilAgent {
    ...
    async assessInfo(context, currentData, userPort, lastDecision, buyingPower, openAccOrders, remainingTrades, orders24h, openPositionCount = 0, geopoliticalSummary = null) {
        ...
        // Built from currentData which is always THIS sweep — never stale.
        const finnhubSignals = (currentData.finnhub?.signals || []).slice(0, 8).join('\n  ') || 'None';
        const mergerNews = (currentData.finnhub?.news?.merger || [])
            .slice(0, 5).map(a => `• ${a.headline} [${a.source || 'Finnhub'}]`).join('\n  ') || 'None';
        const generalNews = (currentData.finnhub?.news?.general || [])
            .slice(0, 4).map(a => `• ${a.headline} [${a.source || 'Finnhub'}]`).join('\n  ') || 'None';
        const maDeals = (currentData.newsapi?.categories?.ma_activity?.articles || [])
            .slice(0, 5).map(a => `• ${a.title} [${a.source || 'NewsAPI'}]`).join('\n  ') || 'None';
        const newsapiSignals = (currentData.newsapi?.signals || []).slice(0, 6).join('\n  ') || 'None';
        const earningsSignals = (currentData.finnhub?.signals || [])
            .filter(s => s.startsWith('EARNINGS')).slice(0, 4).join('\n  ') || 'None';
...
[BREAKING NEWS — THIS SWEEP — FRESH DATA]
M&A / Merger Activity:
  ${mergerNews}
M&A Deals (NewsAPI):
  ${maDeals}
Financial Signals (Finnhub):
  ${finnhubSignals}
NewsAPI Signals:
  ${newsapiSignals}
...
Upcoming Earnings:
  ${earningsSignals}
⚠ If any headline involves a ticker you hold or plan to escalate — treat it as a catalyst or threat immediately.
