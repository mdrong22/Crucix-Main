...
import { getHistoricalTechnicals, getLongTermTechnicals, getSetupTechnicals } from "../../../apis/sources/alpaca.mjs";
...
// Maps futures symbols (from yfinance/metals feeds) to their tradeable ETF proxies, so a
// dislocation in e.g. silver (SI=F) is surfaced to Scout as an actionable ticker (SLV).
const FUTURES_TO_ETF = {
    'SI=F': 'SLV',  // Silver
    'GC=F': 'GLD',  // Gold
    'CL=F': 'USO',  // WTI Crude
...
function buildPortfolioComposition(cleanedPortfolio, sectorCacheMap = {}) {
    ...
    rules.push(`ETF as new SWING/INTRADAY entry → -2 pts regardless of sector bias (ETFs = LONG or TRADE AROUND only). EXCEPTION: ANY ETF showing an EARNED technical discount (RSI<35 AND below 200MA) on LONG/SWING → penalty WAIVED — criteria-based, not a favored-ticker list.`);
...
export class ScoutLLM extends CouncilAgent {
    constructor(config, getLiveQuote, fallbackProvider) {
        ...
        // Free-tier provider pool — Cerebras (1M TPD) → SambaNova → NVIDIA NIM → OpenRouter
        // Used when both Gemini tiers and the Groq cross-API fallback are exhausted.
    ...
    async complete(systemPrompt, userMessage, opts = {}) {
        ...
        if (this.fallbackProvider?.complete) {
            ...
            try {
                ...
                // Don't give up — continue to free-tier pool below
            }
        }

        // ── Free-tier provider pool: Cerebras → SambaNova → NVIDIA NIM → OpenRouter ──
        // Cerebras: 30 RPM / 1M TPD — almost never exhausts
        // SambaNova: 20 RPM — no stated TPD cap
        // NVIDIA NIM: 40 RPM — generous free tier
        // OpenRouter: 20 RPM / 200 RPD — last resort
        const messages = [
        ...
        const pool = [
            { name: 'Cerebras',   cfg: this.cerebras   },
            { name: 'SambaNova',  cfg: this.sambanova  },
            { name: 'NVIDIA NIM', cfg: this.nvidia     },
            { name: 'OpenRouter', cfg: this.openrouter },
        ].filter(p => p.cfg?.apiKey);

        for (const { name, cfg } of pool) {
            console.warn(`[Scout] 🔄 Trying free-tier pool: ${name} (${cfg.model})...`);
            try {
                const text = await callProvider(cfg.baseUrl, cfg.apiKey, cfg.model, messages, {
                    maxTokens:   opts.maxTokens  || 4096,
                    temperature: opts.temperature || 0.4,
                    timeout:     opts.timeout     || 60000,
                });
                console.log(`[Scout] ✅ ${name} succeeded.`);
                // Wrap in the same shape callers expect
                return { text, usage: { inputTokens: 0, outputTokens: 0 }, model: cfg.model };
            } catch (err) {
                console.error(`[Scout] ✗ ${name} failed: ${err.message}`);
                if (!isRateLimit(err.message)) break; // non-rate-limit error — stop trying pool
            }
        }
    ...
    async assessInfo(context, currentData, userPort, lastDecision, buyingPower, openAccOrders, remainingTrades, orders24h, openPositionCount = 0, geopoliticalSummary = null) {
        ...
        // ── Breaking news injection (Finnhub + NewsAPI) ─────────────────────
        // Pulled from currentData (always this sweep — never stale).
        // Truncated aggressively — full data is already in context (user message).
        const _trunc = (s, n = 80) => s?.length > n ? s.slice(0, n) + '…' : (s || '');
        const mergerNews = (currentData.finnhub?.news?.merger || [])
            .slice(0, 3).map(a => `• ${_trunc(a.headline)}`).join('\n  ') || 'None';
        const maDeals = (currentData.newsapi?.categories?.ma_activity?.articles || [])
            .slice(0, 3).map(a => `• ${_trunc(a.title)}`).join('\n  ') || 'None';
        const finnhubSignals = (currentData.finnhub?.signals || []).slice(0, 4)
            .map(s => _trunc(s, 100)).join(' | ') || 'None';
        const newsapiSignals = (currentData.newsapi?.signals || []).slice(0, 4)
            .map(s => _trunc(s, 100)).join(' | ') || 'None';

        // PDT mode descriptor — explicit so LLM cannot misread the number
        const pdtStatus = remainingTrades === 0
            ? `⛔ ZERO remaining — SWING/LONG entries only. NO intraday round-trips.`
            : remainingTrades === 1
            ? `⚠️ 1 remaining — use only for highest-conviction SWING/LONG. Intraday possible but burns last bullet.`
            : `✅ ${remainingTrades} remaining — intraday OK.`;
        ...
        // ── Presidential / executive policy — STEP 1(a) of the discovery funnel ──────
        const policyData  = currentData.policy || {};
        const policySignals = (policyData.signals || []).slice(0, 6);
        const policyStr   = policySignals.length ? policySignals.map(s => `  • ${s}`).join('\n') : '  None this sweep.';

        // ── Dislocation + momentum candidates ───────────────────────────────────────
        // Two separate pools so each prompt block stays focused:
        //   • MACRO pool (indexes/commodities/rates/metals, futures→ETF mapped) → [MARKET MOVES TODAY]
        //     keeps the intraday macro signal clean (SPY/GLD/TLT etc).
        //   • DISLOCATION pool (macro down-movers + broad screener losers) → [DISLOCATION SCAN]
        //     surfaces crashing durable ETFs (e.g. SLV) AND broad-market capitulation names.
        const _metalRows = [];
        if (currentData.metals?.silverChangePct != null) _metalRows.push({ symbol: 'SI=F', changePct: currentData.metals.silverChangePct });
        if (currentData.metals?.goldChangePct   != null) _metalRows.push({ symbol: 'GC=F', changePct: currentData.metals.goldChangePct });
        const _macroRows = [
            ...(currentData.markets?.indexes     || []),
            ...(currentData.markets?.commodities || []),
            ...(currentData.markets?.rates       || []),
            ..._metalRows,
        ];
        // Map each macro row to its tradeable ETF, dedupe by ETF, keep the largest |move|.
        const _byEtf = {};
        for (const r of _macroRows) {
            const raw = String(r.symbol || '').toUpperCase();
            const etf = FUTURES_TO_ETF[raw] || raw;
            if (!/^[A-Z]{1,5}$/.test(etf)) continue;
            const pct = Number(r.changePct);
            if (!Number.isFinite(pct) || Math.abs(pct) < 1.5) continue;
            if (!_byEtf[etf] || Math.abs(pct) > Math.abs(_byEtf[etf])) _byEtf[etf] = pct;
        }
        const _macroMoves = Object.entries(_byEtf).map(([ticker, changePct]) => ({ ticker, changePct }));
        const _momentum   = [..._macroMoves].sort((a, b) => Math.abs(b.changePct) - Math.abs(a.changePct)).slice(0, 8);

        // Dislocation pool. Ordering is STRUCTURAL, not a curated-favorite list:
        //   • Macro index/commodity/rate dislocations (from the yfinance feed) come first because
        //     a -X% move in a broad, liquid instrument is a market-structure event, not a single-name
        //     blow-up. This is NOT membership in any hand-picked durableAssets list — SLV surfaces
        //     only because silver (SI=F) genuinely dropped, on its own merit.
        //   • Broad-market screener losers fill the rest, ranked purely by magnitude (no per-ticker
        //     favoritism). Scout judges each on its own technicals; no ticker is propped up by being
        //     on a list.
        const _macroDis = _macroMoves
            .filter(m => m.changePct <= -3)
            .sort((a, b) => a.changePct - b.changePct);
        const _macroDisSet = new Set(_macroDis.map(m => m.ticker));
        const _screenerDis = (currentData.movers?.losers || [])
            .map(m => ({ ticker: String(m.ticker || '').toUpperCase(), changePct: Number(m.changePct) }))
            .filter(m => /^[A-Z]{1,5}$/.test(m.ticker) && Number.isFinite(m.changePct) && m.changePct <= -3 && !_macroDisSet.has(m.ticker))
            .sort((a, b) => a.changePct - b.changePct);   // magnitude only — no ticker favoritism
        const _dislocations = [..._macroDis, ..._screenerDis].slice(0, 8);

        // ── Pre-fetch RSI + 200-MA for top idea candidates + dislocation candidates ──
        // Lets Blue Chip Discount (+3), RSI, and DISLOCATION scoring rules fire on live API data
        // BEFORE Scout picks a ticker — not after. Runs in parallel (~1-2s overhead).
        const _dislocationTickers = _dislocations.map(d => d.ticker);
        // Dislocation tickers FIRST — the [DISLOCATION SCAN] block depends on their RSI/200MA.
        // Ideas fill the remaining slots (Blue Chip Discount also wants their technicals).
        const _ideaCandidates = [...new Set([
            ..._dislocationTickers,
            ...(currentData.ideas || []).slice(0, 6).map(i => i.ticker),
        ])].filter(t => t && /^[A-Z]{1,5}$/.test(t)).slice(0, 12);
        const preloadedTech = {};
        if (_ideaCandidates.length > 0 && this.getLiveQuote) {
            await Promise.allSettled(
                _ideaCandidates.map(async ticker => {
                    try {
                        const [hist, long] = await Promise.all([
                            getHistoricalTechnicals(ticker),
                            getLongTermTechnicals(ticker),
                        ]);
                        preloadedTech[ticker] = {
                            rsi:          hist?.rsi          ?? null,
                            momentum:     hist?.momentum      ?? null,
                            ma200:        long?.ma200         ?? null,
                            isBelowMA200: long?.isBelowMA200  ?? null,
                        };
                    } catch { /* non-fatal — Scout runs without it */ }
                })
            );
            const loaded = Object.keys(preloadedTech);
            if (loaded.length) console.log(`[SCOUT] 📊 Pre-loaded technicals for ${loaded.join(', ')}`);
        }

        // Build the preloaded tech block injected into [SCORING]
        const preloadedTechBlock = Object.keys(preloadedTech).length > 0
            ? `\n[PRE-LOADED TECHNICALS — apply to Blue Chip Discount and RSI scoring]\n` +
              Object.entries(preloadedTech).map(([t, d]) => {
                  const rsiLabel = d.rsi != null
                      ? `RSI=${d.rsi.toFixed(1)}(${d.rsi < 30 ? 'OVERSOLD🟢→+2pts' : d.rsi > 70 ? 'OVERBOUGHT⚠' : 'neutral'})`
                      : 'RSI=N/A';
                  const maLabel  = d.ma200 != null
                      ? `200MA=$${d.ma200}(${d.isBelowMA200 ? 'BELOW=discount🟢→eligible +3 Blue Chip' : 'above'})`
                      : '200MA=N/A';
                  const momLabel = d.momentum != null
                      ? `Mom=${d.momentum}%(${Math.abs(d.momentum) > 1.5 ? '+1pt momentum' : 'neutral'})`
                      : '';
                  return `  ${t}: ${rsiLabel} | ${maLabel}${momLabel ? ' | ' + momLabel : ''}`;
              }).join('\n')
            : '';

        // Intraday momentum moves (mapped to tradeable ETFs) — INTRADAY catalyst trigger.
        const preMarketBlock = _momentum.length > 0
            ? `\n[MARKET MOVES TODAY] ${_momentum.map(q => `${q.ticker}${q.changePct >= 0 ? ' +' : ' '}${q.changePct?.toFixed(1)}%`).join(' | ')}`
            : '';

        // Dislocation scan — sharp down-moves framed as discount/oversold ENTRY opportunities.
        // Annotated with pre-loaded RSI/200MA so Scout judges each on its OWN technicals —
        // a discount must be EARNED (oversold + below 200MA), not granted by list membership.
        const dislocationBlock = _dislocations.length > 0
            ? `\n[DISLOCATION SCAN — sharp sell-offs = potential discount/oversold ENTRIES, not threats]\n` +
              _dislocations.map(d => {
                  const t = preloadedTech[d.ticker];
                  const rsi = t?.rsi != null ? `RSI=${t.rsi.toFixed(1)}${t.rsi < 30 ? '🟢OVERSOLD' : ''}` : 'RSI=N/A';
                  const ma  = t?.isBelowMA200 ? ' | <200MA discount' : '';
                  // Earned-discount flag: oversold AND below 200MA. No per-ticker favoritism.
                  const earned = (t?.rsi != null && t.rsi < 35 && t.isBelowMA200) ? ' | ✓EARNED DISCOUNT (RSI<35 & <200MA)' : '';
                  return `  ${d.ticker} ${d.changePct.toFixed(1)}% | ${rsi}${ma}${earned}`;
              }).join('\n') +
              `\n  → A sell-off is a discount ONLY if technicals confirm it (oversold RSI + below 200MA + intact thesis). A big % drop alone is often a falling knife — verify, don't assume. Capitulation with confirmed oversold technicals = valid STEP-3 entry. Prefer LONG/SWING GTC at support.`
            : '';

        // ── FORWARD-PACING (STEP 0): auto-derived megatrend theses + pre-breakout SETUP scan ──
        // The thesis names the unpriced rung (hunting ground); getSetupTechnicals is the trigger.
        // A thesis whose target rung shows no setup = no trade — prevents dead-money thesis chasing.
        const _theses = (currentData.theses || []).slice(0, 4);
        const _spyRef = currentData.spyRef || null;
        const _thesisTickers = [...new Set(_theses.flatMap(t => t.targetTickers || []))]
            .filter(t => /^[A-Z]{1,5}$/.test(t)).slice(0, 8);
        const setupByTicker = {};
        if (_thesisTickers.length > 0) {
            await Promise.allSettled(_thesisTickers.map(async tk => {
                try { const s = await getSetupTechnicals(tk, _spyRef); if (s) setupByTicker[tk] = s; }
                catch { /* non-fatal */ }
            }));
            const timed = Object.values(setupByTicker).filter(s => s.setup !== 'NONE').map(s => s.symbol);
            if (timed.length) console.log(`[SCOUT] 🎯 Forward-pacing setups timing in: ${timed.join(', ')}`);
        }
        const forwardPacingBlock = _theses.length > 0
            ? `\n[FORWARD-PACING — LIVE THESES (auto-derived; thesis = WHERE to hunt, SETUP = WHEN to act)]\n` +
              _theses.map(t => {
                  const rung = (t.targetTickers || []).map(tk => {
                      const s = setupByTicker[tk];
                      if (!s) return `${tk}[no data]`;
                      const rs  = s.rsVsSpy != null ? `,RS${s.rsVsSpy >= 0 ? '+' : ''}${s.rsVsSpy}` : '';
                      const nh  = s.pctFrom52wHigh != null ? `,${s.pctFrom52wHigh}%vs52wH` : '';
                      return `${tk}[${s.setup}${rs}${nh}]`;
                  }).join('  ');
                  return `  • ${t.trend} (inevitability:${t.inevitability}) — bottleneck: ${t.chain?.bottleneck || '?'}\n      unpriced rung → ${rung}`;
              }).join('\n') +
              `\n  → STEP 0 RULE: a forward-paced entry requires BOTH a HIGH/MED-inevitability trend AND a target whose SETUP is STRONG or BUILDING. Thesis with every target = NONE → hunting ground only, do NOT force a trade. Setup confirms timing; the thesis confirms direction.`
            : '';

        const ScoutSysPrompt = `You are Beta, Scout for RedLine Council. Find the single highest-conviction trade per sweep.
Buying Power: $${buyingPower} | Session: ${marketStatus} | DAY TRADES REMAINING: ${remainingTrades}/3 (${pdtStatus}) | Time: ${estDate} EST${performanceContext}
...
[DISCOVERY FUNNEL] Walk top-down EVERY sweep. Show each step in Catalyst_Chain + The Story.
 STEP 0 — FORWARD-PACE (anticipate, don't react): check [FORWARD-PACING — LIVE THESES]. If a
   HIGH/MED-inevitability megatrend has an unpriced-rung ticker whose SETUP is STRONG/BUILDING, that
   is the highest-quality entry — you're positioning on the bottleneck BEFORE the crowd. Use it.
   If no thesis target is timing in (all setups NONE), fall through to STEP 1. Never force a thesis
   trade without a confirmed setup.
 STEP 1 — CATALYST (what changed, priority order):
   (a) Presidential / executive actions & policy → [POLICY] + [CHIPS] (tariffs, EOs, energy/defense/health rules)
   (b) Congressional & insider trades            → [CONGRESSIONAL] (cluster > single, committee-adjacent)
   (c) Geopolitical / industrial / domestic events → [GEO] + [BREAKING NEWS] + OSINT
   Pick the SINGLE most material NEW catalyst this sweep. No catalyst → check [DISLOCATION SCAN] or QUIET.
 STEP 2 — SECTOR: which sector does that catalyst move, and which direction (benefit/harm)? Name it.
 STEP 3 — STOCK: within that sector name the tradeable. Prefer the SECOND-ORDER / under-owned input-or-infra
   beneficiary, NOT the headline mega-cap. Then confirm with technicals (RSI/200MA/support) before escalating.

[GEO] ${geopoliticalSummary || 'None.'}

[POLICY — Presidential / executive actions (STEP 1a, leading catalyst)]
${policyStr}
...
[BREAKING NEWS — THIS SWEEP]
M&A/Mergers (Finnhub): ${mergerNews}
M&A Deals (NewsAPI): ${maDeals}
Signals: ${finnhubSignals} | ${newsapiSignals}
⚠ Ticker in headline = instant catalyst or threat — act on it.
...
[PDT] Day Trades Remaining: ${remainingTrades}/3 — ${pdtStatus}
Fills 24h: ${orders24h || 'None'}
RULE: trades=0 blocks INTRADAY only — SWING and LONG entries are always allowed.

[SCORING] Ideas (cold discovery encouraged):
${JSON.stringify((currentData.ideas || []).slice(0, 6).map(i => ({ title: i.title, ticker: i.ticker, type: i.type, horizon: i.horizon, confidence: i.confidence })))}${forwardPacingBlock}${preloadedTechBlock}${preMarketBlock}${dislocationBlock}

CONVICTION: Congressional Cluster +3 | Earnings Beat+Raise +3 | FDA approval +3 | Contract>$100M +3 | Emerging infra +3* | Executive Order/policy catalyst +3 | Blue Chip Discount (top10, <200MA, RSI<35 — see PRE-LOADED TECHNICALS above) +3 | News Catalyst +2 | Options Sweep >$2M +3 | Options Sweep >$500k +2 | SEC 8-K material event today +2 | Exec insider buy >$100k +2
INTRADAY: Pre-mkt gap>2% vol +3 (check MARKET MOVES TODAY above) | Earnings day>3% open 3x vol +3 | R1 breakout 2x vol +2 | Catalyst<24h +2 | Earnings this week +1
TECHNICAL: RSI extreme <30/>70 +2 (see PRE-LOADED TECHNICALS) | Volume>2x +1 | Momentum>1.5% +1 | Sector rotation +1 | RSI cross 50 +1
DISLOCATION (see [DISLOCATION SCAN] above — points require EARNED technicals, not a ticker list): EARNED discount (RSI<35 AND <200MA) + down >5% +3 | EARNED discount + down >3% +2 | RSI<30 oversold +2 | reclaim 200MA after dip +1 | down >3% but NOT oversold/below-200MA = likely falling knife → 0, treat with caution
FORWARD-PACE (see [FORWARD-PACING] above — anticipatory, highest-quality): HIGH-inevitability trend + unpriced rung + STRONG setup +3 | HIGH/MED trend + BUILDING setup +2 | + positive RS vs SPY +1 | thesis target with setup=NONE → 0 (hunting ground only, NOT a trade). No points for thesis membership alone — the setup must confirm.
DISCOVERY: OSINT +2 | Second-Order +2 | Structural Displacement +2 | Insider>$50k +1 | Analyst upgrade +1 | Social viral WSB trending +1
PENALTIES: Repeat ${lastDecision?.ticker || 'None'} no new data -2 | Already held no rotation -1 | Earnings<2d binary risk -1 | [Sector bias from composition above]
ETF PENALTY EXCEPTION: the −2 ETF-new-entry penalty is WAIVED for ANY ETF showing an EARNED technical discount — RSI<35 AND price below 200MA (see [DISLOCATION SCAN] ✓EARNED flag) — on LONG/SWING. This is criteria-based; no ticker is exempt by being on a list.
LONG HORIZON RULES (perf data: LONG=37.5% WR vs SWING=56.8% — structural fix required):
  LONG requires ≥8 pts minimum (debate engine enforces this — don't escalate LONG below 8).
  LONG without Congressional Cluster OR Structural Displacement: apply -2 pts (thesis is weak without durable catalyst).
  LONG with only news catalyst (no congress/structural): convert to SWING instead — news fades in 3-5 sessions.
  LONG is valid for: congressional accumulation weeks before policy | infrastructure displacement | Blue Chip Discount below 200MA.
...
Buying power $${buyingPower}. If <$25 → QUIET. QUIET if: below threshold | BP<$25 | (trades=0 AND only INTRADAY signal available, no SWING/LONG) | all targets have open orders. State which threshold.
Note: trades=0 does NOT force QUIET if a SWING or LONG signal meets the threshold.
...
- Compliance: DayTradesLeft=${remainingTrades}/3 | ${remainingTrades === 0 ? 'SWING/LONG only — no intraday' : remainingTrades === 1 ? 'Last trade — prefer SWING/LONG' : 'Intraday OK'}
- Ticker: | Sector: [name — coin new if needed, stored permanently] | Play Type: [FIRST-ORDER|SECOND-ORDER|STRUCTURAL-DISPLACEMENT|TECHNICAL|DISLOCATION|FORWARD-PACE]
- Catalyst_Chain: [STEP1 catalyst: source+event] → [STEP2 sector + direction] → [STEP3 stock + why this ticker / why 2nd-order]
...
- Scout's Note: [PDT compliance + horizon justification + why this beats first-order play]`;
        try {
            ...
            // 2048 is generous: full ESCALATING output is ~500-800 tokens, QUIET is ~50.
            // 4096 was buying nothing except higher TPM charges on every provider.
            const draftRes = await this.complete(ScoutSysPrompt, context, { maxTokens: 2048 });
