...
export class ScoutLLM extends CouncilAgent {
    ...
    async assessInfo(context, currentData, userPort, lastDecision, buyingPower) {
...
1. PORTFOLIO DEFENSE:
   ...
   - Ticker: [Holding], Trigger: "Averaging Down - Safe Haven Play".
   - You must wait and acknowledge the portfolio before making decisions
...
4. QUIET MODE:
   - Only allowed if NO ideas meet Technical/News triggers OR no viable rotation exists.
...
- **Scout's Note:** [Why this ticker? Why sell the Rotation_Target now? Cite specific %, bars, and momentum tags provided in live data]`;
        try {
