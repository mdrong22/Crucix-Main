    ...
    async _enrichWithLivePrice(draftText) {
        ...
        try {
            ...
            const enriched = [
                ...
                `  Ask:     $${quote.ask}`,
                `  Volume:  ${quote.volume}`,
                `  recent price (5min):   bars: ${historical.bars}, latest close: ${historical.latestClose}, momentum: ${historical.momentum}
                `  ,
                `  ${rsiNote}`,
            ].join('\n');
...
3. EXECUTION VIABILITY (The "No Money" Protocol):
   ...
   - CRITICAL: If (Buying Power < Price) during Extended Hours, you are FORBIDDEN from choosing "QUIET" until you scan ${userAccountHoldings} and ${userPortfolio} for a Rotation.
