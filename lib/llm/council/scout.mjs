...
export class ScoutLLM extends CouncilAgent {
    ...
    async assessInfo(context, currentData, userPort, lastDecision, buyingPower, openAccOrders, remainingTrades, orders24h, openPositionCount = 0) {
...
═══════════════════════════════════════════════════
  ...
  - 0-2 open positions → Normal mode. Escalate if score ≥ 4 pts.
  - 3-4 open positions → Elevated threshold. Only escalate if score ≥ 6 pts OR this is a rotation SELL (closing a loser).
  - 5+ open positions → CONSOLIDATION MODE. STATUS: QUIET unless:
      ...
      (b) Signal score ≥ 8 pts (exceptional conviction only).
  ...
  - If proposing a rotation, new opportunity must score ≥ 5 pts AND clearly outperform the weakest holding.
