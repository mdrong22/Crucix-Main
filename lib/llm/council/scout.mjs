...
DEFENSIVE THRESHOLD: high-prob downside catalyst in today's data, not yet priced (position not already down >8%), specific+imminent only — no vague macro noise. DEFENSIVE beats ESCALATING.
...
[OUTPUT] — Choose ONE of the three statuses below. DEFENSIVE takes priority over ESCALATING.
QUIET: STATUS: QUIET | Reason: [which threshold failed, 1 sentence]

DEFENSIVE:
STATUS: DEFENSIVE
- Ticker: [held symbol at risk]
- Threat: [specific macro/geopolitical event that will cause the drop — be precise, not vague]
- Urgency: [IMMEDIATE = act this session | SWING = act within 1-3 days | WATCH = monitor]
        ...
        try {
            ...
            if (draftText.toUpperCase().includes('ESCALATING')) {
                ...
                if (enrichResult) {
                    ...
                    // Step 3: Mechanically patch the price — LLM anchoring on training data is
                    // too strong to rely on prompt instructions alone. This guarantees the live
                    // price survives into the briefing that Phi, Theta, and Gregor read.
                    if (quote?.price) {
