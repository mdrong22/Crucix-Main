...
export class ScoutLLM extends CouncilAgent {
    ...
    async assessInfo(context, currentData, userPort, lastDecision, buyingPower, openAccOrders, remainingTrades, orders24h, openPositionCount = 0, geopoliticalSummary = null) {
        ...
        // ── Pre-fetch RSI + 200-MA for top idea candidates ───────────────────────────
        // Lets Blue Chip Discount (+3) and RSI scoring rules fire on live API data
        // BEFORE Scout picks a ticker — not after. Runs in parallel (~1-2s overhead).
        const _ideaCandidates = (currentData.ideas || []).slice(0, 6)
            .map(i => i.ticker).filter(t => t && /^[A-Z]{1,5}$/.test(t));
        const preloadedTech = {};
        ...
        // Pre-market / intraday moves from yfinance (already fetched by sweep — zero extra API cost)
        // SPY/QQQ/commodity moves ≥1.5% are the strongest INTRADAY catalyst trigger available
        const _mktMoves = [
         ...
         .slice(0, 8);
        const preMarketBlock = _mktMoves.length > 0
            ? `\n[MARKET MOVES TODAY] ${_mktMoves.map(q => `${q.symbol}${q.changePct >= 0 ? ' +' : ' '}${q.changePct?.toFixed(1)}%`).join(' | ')}`
            : '';
...
${JSON.stringify((currentData.ideas || []).slice(0, 6).map(i => ({ title: i.title, ticker: i.ticker, type: i.type, horizon: i.horizon, confidence: i.confidence })))}${preloadedTechBlock}${preMarketBlock}

CONVICTION: Congressional Cluster +3 | Earnings Beat+Raise +3 | FDA approval +3 | Contract>$100M +3 | Emerging infra +3* | Blue Chip Discount (top10, <200MA, RSI<35 — see PRE-LOADED TECHNICALS above) +3 | News Catalyst +2 | Options Sweep >$2M +3 | Options Sweep >$500k +2 | SEC 8-K material event today +2 | Exec insider buy >$100k +2
