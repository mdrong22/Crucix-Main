...
export class ScoutLLM extends CouncilAgent {
    constructor(config, getLiveQuote, fallbackProvider) {
        ...
        this.fallbackModel = 'gemini-2.5-flash-lite' || null;
        ...
        try {
            ...
            if (this.fallbackModel) {
                ...
                try {
                  ...
                  console.error(`[Scout] ⚠ Fallback model (${this.fallbackModel}) failed: ${err.message}`);
                  if(this.fallbackProvider.model) {
                    console.warn(`[SCOUT] Trying Final Fallback...`)
                    try {
                      return this.fallbackProvider.complete(systemPrompt, userMessage, opts)
                    } catch(err) {
                      console.error(`[REDLINE] Final Fallback Failed ${err.message}`)
                    }}
                }
            }
    ...
    async _enrichWithLivePrice(draftText) {
        ...
        try {
            ...
            const enriched = [
                ...
                `  Ask:     $${quote.ask}`,
                `  Volume:  ${quote.volume}`,
                `  200 Day Moving Average: ${long?.ma200 ?? 'N/A'}`,
                `  Below 200-MA?: ${long?.isBelowMA200 ? 'YES (Discount Zone)' : 'NO'}`,
                `  5min bars — latest close: ${historical?.latestClose ?? 'N/A'}, momentum: ${historical?.momentum ?? 'N/A'}%`,
                `  ${rsiNote}`,
            ].join('\n');
...
═══════════════════════════════════════════════════
SECTION 0.5 — GEOPOLITICAL CONTEXT (Live Intelligence Feed)
═══════════════════════════════════════════════════
${geopoliticalSummary
  ? `Latest geopolitical assessment from the alert evaluator:\n  "${geopoliticalSummary}"\n\n  Use this to:\n  - Adjust risk appetite (escalating conflict = tighten stops, prefer defensive sectors)\n  - Identify macro tailwinds/headwinds for sector selection\n  - Flag if a proposed trade conflicts with active geopolitical risk`
  : `No geopolitical summary available yet — first sweep still pending or no significant signals detected.`}
...
═══════════════════════════════════════════════════
  - Scan ${userPortfolio}. If any holding drops >6% with no confirming news catalyst (i.e., sector-wide or macro-driven drop), set STATUS: ESCALATING with HORIZON: SWING (defensive rotation). Do NOT rotate on normal daily volatility (<6%).
...
═══════════════════════════════════════════════════
  - Cross-reference Ideas: ${JSON.stringify((currentData.ideas || []).slice(0, 6).map(i => ({ title: i.title, type: i.type, ticker: i.ticker, confidence: i.confidence, horizon: i.horizon })))}.
  - IDEAS LIST IS A STARTING POINT, NOT A CONSTRAINT. You may escalate any ticker NOT on the ideas list
    if it scores ≥ 7 pts via other signals. Cold discovery is encouraged — the ideas list can lag real events.
  - SIGNAL SCORING (pick highest composite score):
    ...
    ── CONVICTION SIGNALS ──────────────────────────────────────────────────────
      ...
      * Blue Chip Discount (Top 10 Market Cap & Price < 200-day MA & RSI < 35) = +3 pts

    ── TECHNICAL SIGNALS ───────────────────────────────────────────────────────
      * RSI Extreme (<30 oversold / >70 overbought breakout, mid-caps/momentum runners) = +2 pts
      * Unusual Volume Anomaly (>2x average volume with confirming price move >0.5%) = +1 pt
      * 5m Momentum >1.5% = +1 pt
      * Sector rotation signal = +1 pt

    ── DISCOVERY SIGNALS (cold stock hunting) ──────────────────────────────────
      * OSINT-Derived Catalyst — ticker or sector directly named in the geopolitical summary above
        (Section 0.5), conflict escalation data, or energy/commodity shock with a clear equity
        beneficiary (e.g. defense supplier named in OSINT, LNG exporter during natgas spike) = +2 pts
      * Non-Congressional Insider Buy (corporate insider purchase >$50k, SEC Form 4, within 30 days) = +1 pt
    ...
    ── PENALTIES ───────────────────────────────────────────────────────────────
      ...
      * Ticker already in portfolio with no rotation rationale = -1 pt
...
═══════════════════════════════════════════════════
  - Fractional shares allowed in both REGULAR and EXTENDED sessions. Order type enforcement is Gregor's job — do not constrain sizing here.
  - ROTATION RULE: If Buying Power is low, scan ${userPortfolio} for "Weakest Link".
    A position qualifies as a rotation candidate ONLY IF ALL of the following are true:
      (a) It is down ≥ 3% from entry price — sub-3% losses are noise, NOT a reason to sell.
      (b) The new opportunity scores ≥ 7 pts — same floor as a fresh entry.
      (c) The new opportunity clearly outperforms the weakest holding on a risk-adjusted basis.
    FORBIDDEN: Proposing a rotation on a position that is down less than 3%. A 90-cent loss,
    a 1% dip, or any move within normal daily volatility is NOT a rotation trigger — it is noise.
    If no position meets the 3% threshold, set Rotation_Target: None and use available buying power only.
  - Compliance Check: Rotation_Target was NOT bought today if ${remainingTrades} === 0.

═══════════════════════════════════════════════════
SECTION 6 — POSITION CAP & QUIET MODE TRIGGERS
═══════════════════════════════════════════════════
  OPEN POSITIONS RIGHT NOW: ${openPositionCount}

  POSITION CAP RULES (enforce strictly):
  - 0-2 open positions → Normal mode. Escalate if score ≥ 7 pts.
  - 3-4 open positions → Elevated threshold. Only escalate if score ≥ 7 pts. Rotations do NOT bypass this — the new entry must still score ≥ 7 pts AND the position being sold must be down ≥ 3%.
  - 5+ open positions → CONSOLIDATION MODE. STATUS: QUIET unless:
      (a) You are recommending a rotation that CLOSES an existing position (Rotation_Target ≠ None), OR
      (b) Signal score ≥ 9 pts (exceptional conviction only).
    The portfolio is already overextended. More positions = more noise, not more alpha.

  GENERAL QUIET TRIGGERS:
  - Signal score below the threshold for current position count (see above).
  - Remaining Trades = 0 and no safe SWING/LONG setup exists.
  - All viable targets have existing OPEN_ORDERS.
  - If proposing a rotation, new opportunity must score ≥ 7 pts AND the position being sold must be down ≥ 3%.

═══════════════════════════════════════════════════
OUTPUT FORMAT (STRICT — no deviations)
═══════════════════════════════════════════════════
IF STATUS: QUIET → RETURN ONLY: STATUS: QUIET | Reason: [1 sentence explaining which threshold was not met]
...
- **Signal Score:** [X pts — list each scoring factor that applied and its point value]
