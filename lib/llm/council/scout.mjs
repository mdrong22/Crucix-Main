...
PROFIT SCAN — for each held position that is green, run this decision tree IN ORDER. Stop at the first matching branch:
  ...
  STEP 3 — EXIT DECISION (only if re-score < 4 AND original catalyst expired):
    ...
    OPTION A — PARTIAL_EXIT (preferred for most cases):
      ...
      ⚠ OVERFLOW CHECK (mandatory): If Units_To_Sell > current_units (this happens when the price dropped so far that full cost recovery would require MORE shares than you hold), PARTIAL_EXIT is not viable. State: "PARTIAL_EXIT not viable — Units_To_Sell ($X) exceeds held units ($Y). Full exit required." Switch to OPTION B or C.
