...
export class ScoutLLM extends CouncilAgent {
    ...
    async assessInfo(context, currentData, userPortfolio, userAccountHoldings, lastDecision, buyingPower) {
        ...
        We must grow the portfolio and increase capital from ${buyingPower} in the shortest time if possible. No additional capital will be given. 
        Extended hours (4:00 PM - 8:00 PM EST) (4:00 AM - 9:30 AM EST) You cannot use "notional_value" or fractional decimals (e.g. 0.5).  
        Current date & time: ${estDate} (EST). Remember Market Hours are EST (UTC-5/4)
        ...
        5. MARKET SESSION CONSTRAINTS: 
           Current Session: ${marketStatus}
           
           - IF session is REGULAR_MARKET_OPEN: Fractional/Notional trading is ALLOWED. You may suggest tickers even if Buying Power < Price (using fractional logic).
           - IF session is EXTENDED_HOURS_RESTRICTED: You are FORBIDDEN from suggesting a ticker if (Buying Power / Price) < 1. Whole shares only.
           - During extended hours, If you cannot afford a share of the stock during this time with our buying power ${buyingPower} go into Quiet Mode.
        
        6. CAPITAL ROTATION (No Money, No Problem): 
           If Buying Power (${buyingPower}) is insufficient for a whole share of a high-conviction target during Extended Hours, or if you simply need more fuel:
           - Scan current Holdings: ${userAccountHoldings}.
           - Identify the "Weakest Link": Look for the ticker with the lowest 24h momentum, or a position that has hit its projected profit target.
           - If a rotation is viable, set STATUS: ESCALATING. You MUST specify which ticker to sell to fund the new entry.
        ...
        IF STATUS: QUIET
        STATUS: QUIET
        REASON: [1-sentence explanation of why no ticker was escalated, citing specific constraints if applicable]
        
        IF STATUS: ESCALATING
        STATUS: ESCALATING
        - **Ticker:** [Focus Symbol to BUY]
        - **Rotation_Target:** [Ticker to SELL if applicable, else "None"]
        ...
        - **Scout's Note:** [High-conviction take for the council — cite specific numbers and liquidation reasoning if rotating]`;
