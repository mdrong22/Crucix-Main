...
export async function generateLocalReport(symbol, transcript, scribeInput) {
  ...
  // Parse TRADE REVIEW badge
  const tradeReviewMatch = scribeOutput.match(/TRADE REVIEW:\s*(Needs Review|Neutral|Good)/i);
  const tradeReview      = tradeReviewMatch ? tradeReviewMatch[1] : null;
  const tradeReviewColor = tradeReview === 'Good' ? '#00C896' : tradeReview === 'Neutral' ? '#FFAA00' : tradeReview === 'Needs Review' ? '#FF4444' : null;
  const tradeReviewBadge = tradeReview
    ? `<div class="rl-review-badge" style="background:${tradeReviewColor}22;border:1px solid ${tradeReviewColor};color:${tradeReviewColor};padding:8px 14px;font-size:11px;letter-spacing:2px;font-weight:bold;margin-bottom:16px;display:inline-block;">
        ${tradeReview === 'Needs Review' ? '⚠' : tradeReview === 'Good' ? '✅' : '〜'} ${tradeReview.toUpperCase()}
       </div>`
    : '';
