 ...
 */
/**
 * @param {object} [signalOverrides] - Optional fields to merge into extracted signals.
 *   Use for non-standard flows (e.g. DEFENSIVE exits) where extractSignals can't find
 *   standard Scout fields. Example: { horizon: 'DEFENSIVE', trigger: 'IMMEDIATE', signalScore: null }
 */
export function logDecisions(finalTrades, briefingText, liveVix, remainingTrades, signalOverrides = {}) {
  if (!Array.isArray(finalTrades) || finalTrades.length === 0) return;

  const signals = { ...extractSignals(briefingText, liveVix), ...signalOverrides };
  const existing = loadDecisions();
  const now = new Date();

  // Evaluation deadline by horizon — when positionResolver should force a read
  const horizonDays = { INTRADAY: 1, SWING: 10, LONG: 30, DEFENSIVE: 1, UNKNOWN: 10 };
  ...
  const newEntries = finalTrades.map(trade => ({
    ...
    stopLossPrice:  trade.stopLossPrice ?? null,  // hard stop enforced by stopLossWatcher (bypasses approval)
    signals: {
 ...
 */
export function resolveDecision(id, { outcome, exitPrice, exitTimestamp, pnlPct, pnlDollar, exitReason }) {
...
/**
 * Update arbitrary fields on an open (unresolved) decision.
 * Used by the stop-loss watcher to upgrade horizon (e.g. INTRADAY → SWING).
 * Does NOT mark the decision as resolved.
 */
export function updateDecision(id, fields) {
  const entries = loadDecisions();
  const idx = entries.findIndex(e => e.id === id);
  if (idx === -1) {
    console.warn(`[DecisionLogger] updateDecision: id ${id} not found`);
    return false;
  }
