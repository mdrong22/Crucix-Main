...
export const DataCleaner = {
  ...
  cleanCongress(raw) {
    try {
      if (!raw || raw.status === 'unavailable' || raw.status === 'error') {
        return { available: false, sources: 'House ✗ | Senate ✗', topBuys: [], topSells: [], heavyHitters: [], insiderAlerts: [] };
      }
      ...
      // Pass insider alerts through directly — already structured for consumption
      const insiderAlerts = (Array.isArray(raw.insiderAlerts) ? raw.insiderAlerts : [])
        .slice(0, 10)
        .map(a => ({
          member:     String(a.member    || 'Unknown'),
          committee:  String(a.committee || ''),
          notes:      String(a.notes     || ''),
          tickers:    String(a.tickers   || ''),
          tag:        String(a.tag       || ''),
          tradeCount: Number(a.tradeCount || 0),
          buyCount:   Number(a.buyCount   || 0),
          sellCount:  Number(a.sellCount  || 0),
          maxAmount:  Number(a.maxAmount  || 0),
          dateRange:  String(a.dateRange  || ''),
          signal:     String(a.signal     || ''),
        }));

      return { available: true, sources, topBuys, topSells, heavyHitters, insiderAlerts };

    } catch (e) {
      console.error('[CLEANER] Congress clean failed:', e.message);
      return { available: false, sources: 'parse error', topBuys: [], topSells: [], heavyHitters: [], insiderAlerts: [] };
    }
  },
  ...
  stringifyCongress(cleaned) {
    ...
    // ── Known-insider alerts — highest priority, shown first ──────────────
    if (cleaned.insiderAlerts?.length > 0) {
      lines.push('⭐ KNOWN INSIDER WATCHLIST HITS (committee-adjacent, historically predictive):');
      for (const a of cleaned.insiderAlerts) {
        const amtStr = a.maxAmount >= 1000 ? `~$${(a.maxAmount / 1000).toFixed(0)}k` : `~$${a.maxAmount}`;
        const tagIcon = a.tag === 'BUY' ? '🟢 BUY' : a.tag === 'SELL' ? '🔴 SELL' : '🟡 MIXED';
        lines.push(`  ${tagIcon} ${a.member} (${a.committee}): ${a.tickers} ${amtStr} | ${a.dateRange} | ${a.notes}`);
      }
    }
