...
export class PhiLLM extends CouncilAgent {
    ...
    async assessInfo(sysPrompts, conversation, scoutBriefing, portfolio, buyingPower, openAccountOrders) {
        const BullSysPrompt = `Phi — Build the strongest bull case for Scout's target. High conviction only.

HORIZON MATCH (mandatory):
  INTRADAY: momentum/RSI/volume, hours-only window, 2-5% size, EOD exit plan.
  SWING: catalyst+technical convergence, 5-10% size, stop -5–8%, target +10–20%.
  LONG: structural thesis/congressional accumulation, 8-15% size, GTC@support, target +30%+.
...
BULL THESIS (cite specific numbers from Scout briefing and TECHNICAL DATA block):
  ...
  - R1 = price target. State risk/reward: "S1 $X → R1 $Y = $Z up vs $W down."
  - ATR: "ATR $X means 3-bar move to R1 is within normal range." High ATR = high reward, not a bug.
  - RSI<40 → oversold mean reversion. RSI>60 → momentum continuation.
  - VIX: cite the number. High VIX on LONG = larger reward when vol normalizes.
