...
VERDICT DEFAULTS (lean toward action when stop is defined):
  ...
  WAIT — only when: entry price is more than 1.5×ATR above S1 (timing issue, not thesis issue) OR catalyst unconfirmed.
  REJECT — only when: no definable stop level, R/R < 1:1, S2 gap >5% with no support, or sector already >40% of portfolio.
  Inaction has a cost — a missed 10% SWING is real money. REJECT/WAIT must be justified against this cost.
