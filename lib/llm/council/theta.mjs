 ...
 *  1. Primary model (THETA_MODEL — Qwen3 / QwQ via OpenRouter or Groq)
 *  2. HeyPuter claude-sonnet-4-6 (THETA_PUTER_MODEL) — unlimited, fires when primary rate-limited
 */
...
export class ThetaLLM extends CouncilAgent {
    constructor(config) {
        ...
        if (this.puterApiKey) {
            try {
                ...
                console.log(`[THETA] HeyPuter ready (${this.puterModel})`);
            } catch (e) {
                console.warn('[THETA] ⚠ HeyPuter init failed:', e.message);
                this.puter = null;
            }
        } else {
            console.warn('[THETA] PUTER_AUTH_TOKEN not set — HeyPuter fallback disabled.');
            this.puter = null;
        }
    }

    // ── Tier 2: HeyPuter prose call ──────────────────────────────────────────
    async _tryPuter(systemPrompt, userMessage) {
        if (!this.puter) throw new Error('[THETA] HeyPuter not initialized');
        const messages = [
        ...
        if (!hasUserTurn) messages.push({ role: 'user', content: 'Please review the briefing and provide your risk assessment.' });
    ...
    // ── Tier 1: Primary model (Qwen/QwQ via configured baseUrl) ─────────────
    async complete(systemPrompt, userMessage, opts = {}) {
        ...
        try {
            ...
            console.log(`[THETA] ${text}`);
            return text;

        } catch (err) {
            if (!isRateLimit(err.message)) throw err;
            console.warn(`[THETA] ⚠ Primary model (${this.model}) hit limit: ${err.message}`);
        }

        // ── Tier 2: HeyPuter (unlimited) ─────────────────────────────────────
        if (this.puter) {
            console.warn(`[THETA] Switching to HeyPuter (${this.puterModel})...`);
            const text = await this._tryPuter(systemPrompt, userMessage);
            console.log(`[THETA] (puter) ${text}`);
            return text;
        }

        throw new Error(`[THETA] All models exhausted — primary rate-limited and no PUTER_AUTH_TOKEN set.`);
    }
