...
export class GregorLLM extends CouncilAgent {
    ...
    async assessInfo(sysPrompts, conversation, buyingPower, totalValue, orders24h, vix = 'N/A', opts = {}) {
        ...
        Context: Buying Power: ${buyingPower} | Total Value: ${totalValue} | Market Time: ${new Date().toISOString()}} to (EST).
        ...
        4. Position Sizing: Calculate units based on buying power. If you want a fractional amount, you MUST provide "notational_value" (the USD amount) and you may set "units" to null.
        ...
        VERDICT: {"action":"BUY"|"SELL"|"WAIT","order_type":"Limit"|"Market","time_in_force":"Day"|"GTC","price":NUMBER_OR_NULL,"units":POSITIVE_NUMBER_OR_NULL,"notational_value":NUMBER_OR_NULL,"trading_session":"REGULAR"|"EXTENDED"}
        
        Example:
        TARGET: BA
        Logic: Defense contracts and low RSI outweigh current VIX levels.
        VERDICT: {"action":"BUY","order_type":"Limit","time_in_force":"GTC","price":190.52,"units":null,"notational_value":100,"trading_session":"EXTENDED"}`;
