...
export class GregorLLM extends CouncilAgent {
    ...
    async assessInfo(sysPrompts, conversation, buyingPower, totalValue, orders24h, vix = 'N/A', openAccountOrders, portfolio) {
        const now = new Date();
        ...
        5. Position Sizing: If you want to trade a specific dollar amount (Fractional), use "notional_value" and set "units" to null. This ONLY works for "order_type":"Market".
        6. Sell Constraints: If "order_type" is "Limit" (required for EXTENDED sessions), you CANNOT use "notional_value". You MUST calculate and provide "units" based on the "Current Positions" and set "notional_value" to null. Also Prefer "time_in_force":"Day" to release frozen capital unless there is a logical reason to not do so.
        ...
        The JSON must start on the same line as the word "VERDICT:" and must be an array of order objects.
        ...
        Example for multiple orders:
        TARGET: BA
        Logic: Defense contracts and low RSI outweigh current VIX levels. Existing positions allow for a partial limit sell.
        VERDICT: [{"action":"SELL","order_type":"Limit","time_in_force":"GTC","price":190.52,"units":5.5,"notional_value":null,"trading_session":"EXTENDED"}]`;
