...
export class GregorLLM extends AnthropicProvider {
    constructor(config) {
        // Pass Anthropic fallback keys to super so this.complete() calls Anthropic correctly
        super({
            apiKey: config.fallback?.apiKey || null,
            model:  config.fallback?.model  || 'claude-opus-4-6',
        });
        this.name       = 'gregor';
        this.puter      = init(config.apiKey);  // HeyPuter — primary execution path
        this.puterModel = config.model;         // e.g. PUTER_MODEL from .env
    }

    /**
     * Primary execution path via HeyPuter.
     * On any failure, falls back to this.complete() → Anthropic API.
     */
    async usePuter(systemPrompt, userMessage) {
        ...
        try {
            ...
            console.error('[GREGOR] ❌ HeyPuter failed:', err.message);
            console.warn('[GREGOR] Falling back to Anthropic...');
            // this.complete() now uses Anthropic keys from super() constructor
            const fallback = await this.complete(systemPrompt, userMessage, { maxTokens: 4096, temperature: 0.4 });
...
Logic:
  Bull Case Assessment: [Why Phi's specific argument is compelling — cite the actual catalyst, price level, or data point that makes this actionable TODAY. Not a generic statement.]
  Bear Case Response: [Address Theta's specific risk concern. Did you override it? Why? What specific factor tips the balance toward execution despite the risk?]
  PDT & Timing: [How many day trades remain. Does this horizon respect PDT? If 0 trades remain and this is INTRADAY — stop here and issue WAIT instead.]
  Portfolio Context: [Buying power utilization, concentration check result, idle capital consideration. Is deploying capital here justified given current portfolio state?]
  Sizing Rationale: [Why you chose this specific notional/units amount. Which formula tier applies (INTRADAY/SWING/LONG). Any performance modifier from last review applied?]
  Final Conviction: [One sentence — the decisive reason you are executing vs. standing down.]
