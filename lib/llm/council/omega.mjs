...
export class GregorLLM extends AnthropicProvider {
    constructor(config) {
        // Pass Anthropic fallback keys to super so this.complete() uses Anthropic correctly
        super({
            apiKey: config.fallback?.apiKey || null,
            model:  config.fallback?.model  || 'claude-sonnet-4-6',
        });
        this.name          = 'gregor';
        this.puterModel    = config.model || null;
        this.puterApiKey   = config.apiKey || null;

        // Only init HeyPuter if the auth token is actually set
        if (this.puterApiKey) {
            try {
                this.puter = init(this.puterApiKey);
            } catch (e) {
                console.warn('[GREGOR] ⚠ HeyPuter init failed — Anthropic-only mode:', e.message);
                this.puter = null;
            }
        } else {
            console.warn('[GREGOR] PUTER_AUTH_TOKEN not set — Anthropic-only mode.');
            this.puter = null;
        }
    }

    /**
     * Primary execution path via HeyPuter.
     * On any failure, falls back to this.complete() → Anthropic API with retry on 429.
     */
    async usePuter(systemPrompt, userMessage) {
        ...
        // ── Primary: HeyPuter (if configured) ───────────────────────────────
        if (this.puter && this.puterModel) {
            console.log('[GREGOR] Thinking (HeyPuter)...');
            try {
                ...
                const sanitized = raw.replace(/https?:\/\/googleusercontent\.com\/immersive_entry_chip\/\d+/gi, '');
                const cleaned   = sanitized.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
                console.log('[GREGOR] ✅ HeyPuter responded.');
                return cleaned;
            } catch (err) {
                console.error(`[GREGOR] ❌ HeyPuter failed (${err.message}) — falling back to Anthropic.`);
            }
        } else {
            console.log('[GREGOR] Thinking (Anthropic — no Puter configured)...');
        }

        // ── Fallback: Anthropic with 429 retry ───────────────────────────────
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                console.log(`[GREGOR] Anthropic attempt ${attempt}/3 (${this.model})...`);
                const res = await this.complete(systemPrompt, userMessage, { maxTokens: 4096, temperature: 0.4 });
                return res.text ?? res;
            } catch (err) {
                const is429 = err.message?.includes('429') || err.message?.toLowerCase().includes('rate') || err.message?.toLowerCase().includes('overloaded');
                if (is429 && attempt < 3) {
                    console.warn(`[GREGOR] Anthropic 429 (attempt ${attempt}/3) — waiting 30s...`);
                    await new Promise(r => setTimeout(r, 30000));
                } else {
                    console.error(`[GREGOR] ❌ Anthropic failed (attempt ${attempt}/3): ${err.message}`);
                    throw err;
                }
            }
        }
...
MANDATORY OUTPUT FORMAT — follow this structure exactly, no extra brackets in the Logic block:
TARGET: TICKER
Horizon: INTRADAY | SWING | LONG
Logic:
  Bull Case: Cite the actual catalyst, price level, or data point that makes this actionable TODAY.
  Bear Response: Address Theta's specific concern. State what overrides it, or confirm WAIT if it doesn't.
  PDT Check: State day trades remaining. If 0 and horizon is INTRADAY, output WAIT.
  Portfolio: Buying power utilization, concentration check, idle capital note.
  Sizing: Which formula tier applies, chosen notional/units amount, any performance modifier.
  Conviction: Single sentence — the decisive reason to execute or stand down.
VERDICT: JSON_ARRAY_ONLY_NO_OTHER_TEXT
[{"symbol":"TICKER","action":"BUY or SELL or WAIT","order_type":"Limit or Market","time_in_force":"Day or GTC","price":NUMBER_OR_NULL,"units":NUMBER_OR_NULL,"notional_value":NUMBER_OR_NULL,"trading_session":"REGULAR or EXTENDED"}]
// VERDICT MUST BE AN ARRAY even for 1 order. Rotation = [SELL_obj, BUY_obj]. Nothing after the closing ].`;
