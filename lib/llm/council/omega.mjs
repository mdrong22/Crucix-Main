...
export class GregorLLM extends AnthropicProvider {
    constructor(config) {
        ...
        this.fallbackModel = config.fallback.model || 'claude-sonnet-4'
    }

    async usePuter(systemPrompt, userMessage, models) {
...
// VERDICT MUST BE AN ARRAY — even for a single order. Rotations = [SELL_obj, BUY_obj].`;
        ...
        try {
            console.log('[GREGOR] Thinking...');
            const res = await this.usePuter(GregorSysPrompt, fullConversation, this.model);
            return res.text ?? res;
        } catch (err) {
            console.error("[RedLine] Gregor failed to reach a verdict:", err.message);
            const res = await this.usePuter(GregorSysPrompt, fullConversation, this.fallbackModel);
        }
    }
}
