...
// ── Extract + validate the VERDICT JSON array from any model's raw text ──────
function extractVerdict(raw) {
    // Strip think-tags first (DeepSeek R1, Qwen3 reasoning output)
    let text = raw.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();

    // Try to find VERDICT: [...] pattern first
    const verdictMatch = text.match(/VERDICT\s*:\s*(\[[\s\S]*?\])/i);
    if (verdictMatch) {
        try { return JSON.parse(verdictMatch[1]); } catch { /* fall through */ }
    }

    // Fall back: find the last [...] in the response
    const lastBracket = text.lastIndexOf(']');
    if (lastBracket !== -1) {
        const firstBracket = text.lastIndexOf('[', lastBracket);
        if (firstBracket !== -1) {
            try { return JSON.parse(text.substring(firstBracket, lastBracket + 1)); } catch { /* fall through */ }
        }
    }

    // Absolute fallback — return the full text so debate.mjs can try its own extractor
    return text;
}
    ...
    async _callChain(systemPrompt, userMessage) {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                return extractVerdict(res?.text ?? res);
            } catch (err) {
...
Logic:
  ...
  Bear Response: Address Theta's specific concern and explain why it is outweighed.
  ...
  Sizing: Which formula tier applies and the exact notional or unit calculation.
