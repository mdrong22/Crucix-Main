  ...
  async complete(systemPrompt, userMessage, opts = {}) {
    ...
    try {
      return await new Promise((resolve, reject) => {
        ...
        child.on('close', code => {
          clearTimeout(timer);
          // A usage-limit / auth failure exits non-zero with no API call — surface it clearly so
          // it's obvious this is a quota window, not a code bug. The Gemini fallback covers it.
          const blob = (out + err).toLowerCase();
          if (blob.includes('usage limit') || blob.includes('rate limit') || blob.includes('quota') || blob.includes('exceeded')) {
            return reject(new Error('claude CLI hit a Claude Code usage limit — falling back (subscription quota window).'));
          }
