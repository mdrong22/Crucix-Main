...
import { ClaudeCodeProvider } from "./claudeCode.mjs";

export { LLMProvider } from "./provider.mjs";
export { ClaudeCodeProvider } from "./claudeCode.mjs";
...
export function createLLMProvider(llmConfig) {
  ...
  const { provider, apiKey, model, fallbackModel, baseUrl } = llmConfig;

  switch (provider.toLowerCase()) {
    case "claude-code":
    case "claude-cli":
    case "claudecode":
      // Runs the agent on the Claude Code SUBSCRIPTION via the `claude` CLI — no API credits.
    ...
    case "gemini":
      // fallbackModel forwarded so config.llm.fallbackModel is actually used
