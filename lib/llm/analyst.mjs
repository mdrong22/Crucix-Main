...
{"action":"NEW_BUY|MAINTENANCE|NO_ACTION","title":"","desc":"","ticker":"","side":"BUY|SELL","order_type":"Limit|Market","price":0,"units":0,"notional_value":null,"time_in_force":"Day|GTC","horizon":"INTRADAY|SWING|LONG","stopLoss":0,"confidence":"HIGH|MEDIUM|LOW","expiresInMinutes":60}
...
export function buildAnalystContext(currentData, portfolio, openOrders, buyingPower, remaining, priorPending = [], allowedHorizons = ['INTRADAY', 'SWING', 'LONG']) {
  let sweep = '';
 ...
 */
export async function generateProposal(provider, currentData, portfolio, openOrders, buyingPower, remaining, priorPending = [], fallbackProvider = null, allowedHorizons = ['INTRADAY', 'SWING', 'LONG']) {
  if (!provider?.isConfigured) return null;
  const context = buildAnalystContext(currentData, portfolio, openOrders, buyingPower, remaining, priorPending, allowedHorizons);
