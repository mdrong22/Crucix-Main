...
Output ONLY valid JSON array. Each object:
{
  ...
  try {
    ...
    console.warn(`[LLM Ideas] Primary provider failed: ${err.message} — trying fallback.`);
  }

  // ── Fallback provider (Groq / other fast provider) ─────────────────────────
  if (fallbackProvider?.isConfigured) {
    try {
      console.log(`[LLM Ideas] ⚡ Retrying with fallback: ${fallbackProvider.name} / ${fallbackProvider.model}`);
      const result = await fallbackProvider.complete(systemPrompt, context, { maxTokens: 4096, timeout: 45000 });
      const ideas = parseIdeasResponse(result.text);
      if (ideas && ideas.length > 0) {
        console.log(`[LLM Ideas] ✅ Fallback succeeded — ${ideas.length} ideas generated.`);
